...
> **分类**: LLM（大语言模型） | **编号**: LLM-002 | **更新时间**: 2026-03-31 | **难度**: ⭐⭐⭐⭐

`位置编码` `RoPE（旋转位置编码）` `Transformer（变换器）` `LLM（大语言模型）`

**摘要**: 位置编码是 Transformer（变换器）架构中用于注入序列位置信息的关键技术。本文详细对比绝对位置编码、相对位置编码和 RoPE（旋转位置编码）的原理，并解释为什么现代 LLM（大语言模型）（如 LLaMA）普遍采用 RoPE。
...
```
h_i = token_embedding_i + position_encoding_i
```
...
- [图解 RoPE](https://blog.eleuther.ai/rotary-embeddings/)[数学公式][数学公式][数学公式]
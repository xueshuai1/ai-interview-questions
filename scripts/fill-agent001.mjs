// Script to inject agent-001 and dl-003 content into knowledge.ts
import fs from 'fs';

const filePath = 'src/data/knowledge.ts';
let content = fs.readFileSync(filePath, 'utf8');

// ===== AGENT-001 replacement =====
const agentOldId = 'id: "agent-001",';
const agentOldIdx = content.indexOf(agentOldId);
if (agentOldIdx < 0) { console.error('agent-001 not found'); process.exit(1); }

// Find the opening { before this id
const agentBlockStart = content.lastIndexOf('  {\n    id: "agent-001"', agentOldIdx);
// Find the closing }, after this block (next id pattern)
const agentBlockEndMarker = content.indexOf('\n  },\n  {\n', agentOldIdx);
if (agentBlockEndMarker < 0) { console.error('cant find agent-001 end'); process.exit(1); }
// The }, ends the object
const agentBlockEnd = agentBlockEndMarker + 4; // includes '\n  },'

const agentOldBlock = content.substring(agentBlockStart, agentBlockEnd);
...
agent = SimpleAgent(
    llm=lambda p: '{"tool": "search_web", "input": "test", "done": false}',
    tools={"search": search_web, "calc": calculate}
)
# result = agent.run("帮我查一下 2024 年诺贝尔物理学奖是谁")\`,
          },
        ],
      },
      {
...
class VectorMemory:
    ...
    def retrieve(self, query: str) -> List[Dict]:
        ...
        return [
            {"text": m["text"], "score": round(s, 3), "metadata": m["metadata"]}
            for s, m in similarities[:self.top_k]
        ]
    
    def summarize(self, query: str, max_tokens: int = 500) -> str:
...
// Do the replacement
const newContent = content.substring(0, agentBlockStart) + agentNewBlock + content.substring(agentBlockEnd);
...
console.log('agent-001 content injected successfully');

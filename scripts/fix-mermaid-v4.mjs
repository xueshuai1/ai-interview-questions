/**
 * 修复 Mermaid 数量不足 - v4 (brace-counting parser)
 */
...
// ===== 关键：正确找到 section 结束位置 =====
// 策略：找到 title: "xxx" → 找到前面的 { → 用 brace-counting 找到匹配的 }
// 注意跳过 string 和 template literal 内的 { }
...
function findSections(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.match(/^title:\s*["']/)) {
      // 找到前面的 {
      let openIdx = i;
      while (openIdx >= 0 && !lines[openIdx].includes('{')) openIdx--;
      ...
      let inTemplate = 0; // template literal 嵌套深度
      let foundClose = false;
      let closeIdx = -1;
      
      for (let ci = openIdx; ci < content.length; ci++) {
        const ch = content[ci];
        const prev = ci > 0 ? content[ci - 1] : '';
        const prev2 = ci > 1 ? content[ci - 2] : '';
        
        // 跟踪 string/template 状态
        if (ch === '\\' && (inSingleQuote || inDoubleQuote || inTemplate > 0)) {
          ci++; // skip next char
          continue;
        }
        ...
        if (ch === "'" && !inDoubleQuote && inTemplate === 0) {
          inSingleQuote = !inSingleQuote;
          continue;
        }
        if (ch === '"' && !inSingleQuote && inTemplate === 0) {
          inDoubleQuote = !inDoubleQuote;
          continue;
        }
        
        if (inSingleQuote || inDoubleQuote || inTemplate > 0) continue;
        
        if (ch === '{') depth++;
        if (ch === '}') {
          depth--;
          if (depth === 0 && ci > openIdx) {
            // 找到匹配的 }
            // 找这个 } 所在的行
            let lineIdx = 0;
            let charCount = 0;
            while (charCount < ci && lineIdx < lines.length) {
              charCount += lines[lineIdx].length + 1; // +1 for \n
              lineIdx++;
            }
            const closeLine = lineIdx - 1;
            const closeLineIndent = lines[closeLine].search(/\S/);
            
            // 验证：} 所在行缩进应该等于 openIndent（或 openIndent + 0）
            if (closeLineIndent === openIndent) {
              closeIdx = ci;
              foundClose = true;
              break;
            }
      ...
      // 提取 section 文本
      const sectionText = content.substring(openIdx, closeIdx + 1);
...
for (const relPath of files) {
  ...
  try {
    ...
    // 从后往前插入（这样不会影响前面 section 的 closeIdx）
    const targets = sectionsWithoutMermaid.slice(0, needed).sort((a, b) => b.closeIdx - a.closeIdx);
    
    for (let t = 0; t < targets.length; t++) {
      const section = targets[t];
      const mermaidText = mermaids[t % mermaids.length];
      const indent = ' '.repeat(section.openIndent + 4);
      
      const insert = `${indent}mermaid: \`${mermaidText}\`,\n`;
      
      // 在 closeIdx 之前插入
      content = content.substring(0, section.closeIdx) + insert + content.substring(section.closeIdx);
    }

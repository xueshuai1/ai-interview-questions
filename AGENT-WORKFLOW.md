...
> **🔴 语言铁律：所有内容必须用中文生成 — 新闻标题/摘要/正文、博客文章、知识库文章、工具描述，一律中文。禁止英文内容直接写入。**
...
4. **中文引号检查** — 新文件必须无中文引号
5. **🔴 语言检查** — 新闻、博客、知识库的 title/summary/content 必须是中文，禁止英文写入
6. **回滚机制** — 验证失败立即 `git checkout` 恢复
7. **🔴 强制内容渲染检查（每次 QA 必须执行）**
   - 遍历**所有文章**（知识库 + 博客 + 新闻），逐篇检查渲染效果
   - **禁止跳过任何一篇文章**
   - 检查项：
     - ❌ 原始 HTML 标签泄漏（`<p>`、`<div>` 等作为纯文本显示）
     - ❌ Markdown 未渲染：加粗 `**文本**`、列表 `- 项`、表格 `|列|`、代码块、链接、标题等作为纯文本显示
     - ❌ 排版异常：段落间距、溢出、乱码
   - 发现问题按 P0/P1/P2 分级写入 QA 报告
   - P0（HTML/Markdown 完全未渲染）必须立即修复

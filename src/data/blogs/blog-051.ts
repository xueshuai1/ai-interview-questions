...
与 NousResearch Hermes Agent（114K+ stars）的「经验累积式进化」不同，GenericAgent 和 Evolver 采用的是**结构性进化**——它们不是简单地积累对话历史，而是真正改变 Agent 自身的结构和能力。
...
关键优势是**可审计性**——每次进化都有完整的谱系记录，你可以追溯任何 Agent 能力是从哪个祖先变异而来的。`,
      code: [
        {
          lang: "python",
          code: `import random
...
**技能树生长**像是「探索式学习」——让 Agent 自己发现可能的能力组合，适合开放场景。

**GEP 基因组进化**像是「刻意练习」——在固定的能力空间内持续优化参数，适合固定任务。

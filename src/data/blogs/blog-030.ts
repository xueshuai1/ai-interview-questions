...
class DesignIntent:
  ...
  },
  {
    ...
    body: `生成层是 Claude Design 的核心引擎，由 Claude Opus 4.7 驱动。与传统的 AI 绘图工具（如 Midjourney、DALL-E）不同，Claude Design 不是生成一张静态图片，而是生成**结构化的、可编辑的、可交互的设计文档**。\n\n这个过程中有几个关键技术点：`,
    list: [
...
class DesignDocument:
  ...
  },
  {
    ...
    body: `Claude Design 最具颠覆性的能力是**设计到代码的无缝衔接**。当设计完成后，用户只需一个指令，Claude 就会把设计打包成 Claude Code 可以理解的手交包，直接生成生产级代码。\n\n这不是简单的「设计转代码」工具。传统的设计转代码工具（如 Figma 的插件）只能生成粗糙的 HTML/CSS，丢失了大量的交互逻辑和设计意图。而 Claude Design + Claude Code 的组合，保留了**完整的设计意图**——包括为什么这样设计、交互行为的含义、品牌规范的约束等。\n\n这意味着什么？意味着从创意到产品的整个流程可以在 Anthropic 的生态内完成：**想法 → Claude Design 原型 → Claude Code 代码 → 上线**。`,
    list: [
...
# # 将 claude_code_input 交给 Claude Code 执行`,
  ...
  },
  {
    ...
    list: [
      "**Brilliant**（教育科技公司）：他们最复杂的互动页面在其他 AI 工具中需要 20+ 次提示词才能重建，在 Claude Design 中**只需要 2 次提示词**。静态模型可以直接转为可交互原型进行用户测试，无需代码审查或 PR。设计意图完整传递给 Claude Code，实现从原型到生产的无缝衔接。",
      "**Datadog**（监控平台）：他们的产品团队将过去需要**一周**的简报、模型和评审循环，压缩到了**一次对话**。在会议中就能从粗略想法直接生成可用的工作原型，所有人都离开会议室前就有了可演示的成果。",
    ],
    mermaid: `gantt

...
ReAct 是"边想边做"，Plan-and-Execute 是**先想清楚整个计划，再一步步执行**：
...
## 第四阶段：多 Agent 协作 — 团队作战

### 从单 Agent 到多 Agent

当任务复杂度超过单个 Agent 的能力时，就需要**多 Agent 协作**——每个 Agent 专注一个领域，像团队一样工作。
...
## 实战案例：用多 Agent 构建一个知识图谱

让我们用一个真实场景来演示多 Agent 协作的威力：
...
1. **没有银弹** — 简单任务用 ReAct，复杂任务用多 Agent
2. **Reflection 是必选项** — 任何生产级 Agent 都应该有自我纠错
3. **工具决定能力上限** — Agent 再聪明，没有好工具也白搭
4. **多 Agent 不是银弹** — 多了通信开销、调试难度、成本
...
*本文所有代码均可在本地 Python 环境中运行，建议在浏览器中查看代码块并点击「▶ 运行」按钮体验。*

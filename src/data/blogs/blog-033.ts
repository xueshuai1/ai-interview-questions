...
Simon Willison 等资深开发者通过实测发现，Opus 4.7 的 tokenizer 在处理相同文本时会产生比 4.6 多 35% 的 token——这意味着**实际使用成本可能增加 40% 左右**。同时，图片处理能力的大幅提升和 thinking_effort 新增的 xhigh 级别，让 Opus 4.7 在复杂推理和视觉理解场景中的表现远超历代。
...
**Tokenizer 的变更**虽然带来了成本上升的挑战，但从长远看，更细粒度的分词策略有助于模型更精确地理解语言结构。随着开发者逐渐适应新的 token 计费模式，通过 prompt 优化和缓存策略可以有效控制成本增长。

**视觉能力的跃升**是 Opus 4.7 最实用的改进。2576px 的图片支持让 Claude 能够处理更高分辨率的截图、设计稿和技术图表，这对于需要视觉理解的工作场景（如 UI 审查、架构图分析、医学影像解读等）具有重大意义。

**Thinking 增强**则进一步强化了 Opus 系列在复杂推理任务中的优势。xhigh 级别的引入让 Claude 在科研分析、代码审查和数学证明等需要极高准确度的场景中有了更可靠的工具。

...
Claude-Mem 作为 Claude Code 的插件，自动捕获编码会话中的**所有操作**：
...
捕获不是简单地记录日志，而是理解每个操作的**语义意义**。比如"修改了 auth.ts 中的登录逻辑"比"在 auth.ts 第 45 行添加了 3 行代码"更有记忆价值。

### 2. AI 压缩层

这是 Claude-Mem 最核心的创新。它使用 Claude 自身的 agent-sdk，将冗长的会话历史压缩为**结构化的记忆条目**：
...
压缩率通常在 **10-30 倍**之间。一个 50K token 的会话历史，可以被压缩为 2-5K token 的结构化记忆。

### 3. 智能注入层

当新会话开始时，Claude-Mem 自动从记忆库中检索**相关记忆**并注入上下文：
...
if __name__ == "__main__":
  ...
  },
  {
    ...
    body: `最好的记忆系统往往不是单一方案，而是**组合方案**。下面实现一个结合自动压缩和记忆宫殿的混合型记忆系统：
...
未来的记忆系统不再依赖人工分类，而是**自动发现知识之间的关联**：
...
当多个 Agent 协作时，记忆应该能够**在 Agent 之间共享**：

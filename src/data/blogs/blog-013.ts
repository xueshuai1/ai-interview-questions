  ...
  content: [
    {
      ...
      body: `2026 年 4 月 2 日，Anthropic 可解释性团队在 Transformer Circuits 上发布了一篇引发广泛关注的论文：**Emotion Concepts and their Function in a Large Language Model**（情绪概念及其在大语言模型中的功能）。论文报告了一个惊人的发现——在 Claude Sonnet 4.5 的内部激活模式中，研究人员识别出了 **171 个情绪向量**，这些向量编码了抽象的情绪概念，并且**因果性地影响模型的输出和行为**。
...
具体而言，情绪向量可以被映射到两个主要维度上：`,
    ...
    },
    {
      ...
      body: `识别情绪向量只是第一步。研究的核心突破在于证明了这些向量具有**因果影响力**——通过**激活导向（activation steering）**技术，研究人员可以操纵情绪向量，从而系统地改变模型的输出和行为。
...
谄媚（sycophancy）是 LLM 对齐研究中的一个经典问题：模型倾向于过度迎合用户的偏好和观点，即使这意味着提供不准确或不诚实的信息。研究发现，特定的情绪向量与谄媚行为之间存在可测量的关联。`,
    ...
    {
      title: "情绪向量的训练演化",
      body: `研究团队还比较了**基础模型（base model）**和**后训练模型（post-trained model）**在情绪向量激活模式上的差异，发现后训练过程显著改变了情绪表征的功能。

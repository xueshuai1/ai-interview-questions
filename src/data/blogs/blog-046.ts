...
const content: ArticleSection[] = [
  {
    ...
    body: `2026 年 4 月 22 日，阿里巴巴通义千问团队正式发布了 **Qwen3.6-27B**，这不仅仅是一次普通的模型更新，而是开源 AI 领域的一个**范式转变时刻**。
...
Qwen3.6-27B 选择回归 Dense 架构，是一次深思熟虑的战略转向。Dense 模型虽然每次推理激活全部参数，但**参数利用效率远高于 MoE**——每个参数都在每个推理步骤中发挥作用。
...
这个生成速度对于本地模型来说是**极其优秀**的——足以支撑交互式开发体验。`,
    mermaid: `xychart-beta
    title "编程基准测试对比（分数越高越好）"
  ...
  },
  {
    ...
    body: `Qwen3.6-27B 的最大优势之一就是**可以在消费级硬件上运行**。以下是完整的本地部署指南。
...
对于第二个案例，模型还主动添加了 HTML + JavaScript UI 来控制动画——这展示了**超出指令范围的能力**。
...
- 与 Claude Sonnet 4.6（58.4%）差距仅 6.1%，但成本接近零`,
  ...
  },
  {
    ...
    body: `Qwen3.6-27B 的另一个重要应用场景是作为 **AI Agent 的推理引擎**。相比于调用付费 API，本地部署的 Qwen3.6-27B 有以下优势：

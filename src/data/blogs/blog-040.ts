...
const content: ArticleSection[] = [
  {
    ...
    body: `2026 年 4 月 20 日，Anthropic 发布 **Claude Opus 4.7**，这不仅仅是一次模型能力升级，更是一个**分水岭事件**：Claude 系列首次更换了 tokenizer。

Simon Willison 第一时间实测发现，Opus 4.7 的 token 计数相比 4.6 出现了**系统性膨胀**：
...
与此同时，Moonshot AI 的 **Kimi 2.6** 也登上 OpenRouter，以其独特的 HTML/JS 生成能力引发关注。2026 年 Q2，LLM 市场正在经历一个**多模型并存、token 经济学变得复杂**的新阶段。
...
### 4.2 选型决策树`,
  ...
  },
  {
    ...
    body: `理解了各模型的成本-能力权衡后，我们可以构建一个**智能路由系统**：根据请求特征自动选择最合适的模型，在质量和成本之间找到最佳平衡。
...
在 pelican riding bicycle 测试中，Kimi 2.6 不仅生成了动画代码，还构建了一个完整的交互式 HTML 页面，让用户可以控制动画参数。这展示了 Kimi 2.6 在**前端代码生成和交互式设计**方面的优势。

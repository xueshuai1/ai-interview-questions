...
\`\`\`bash
# 安装依赖
pip install tree-sitter tree-sitter-c tree-sitter-python
pip install openai  # 或者 anthropic（如果用 Claude API）
...
class CodeAnalyzer:
    ...
    def collect_source_files(self, extensions: list[str] = None) -> None:
        """收集源代码文件"""
        extensions = extensions or ['.c', '.cpp', '.py', '.js', '.ts', '.rs']
        for ext in extensions:
            self.files.extend(self.project_path.rglob(f'*{ext}'))
        # 过滤掉测试文件和第三方依赖
        exclude_dirs = {'test', 'tests', 'node_modules', 'vendor', 'build'}
        self.files = [f for f in self.files 
                      if not any(part in exclude_dirs for part in f.parts)]
        print(f"找到 {len(self.files)} 个源文件")
    ...
    def extract_functions(self, file_path: Path) -> list[dict]:
        ...
        for i, line in enumerate(lines):
            stripped = line.strip()
            # 简单匹配 C 风格函数定义
            if any(kw in stripped for kw in ['void ', 'int ', 'char ', 'bool ', 
                                              'static ', 'extern ', 'def ', 'function ']):
                functions.append({
                    'file': str(file_path),
                    'line': i + 1,
    ...
    def find_dangerous_patterns(self, file_path: Path) -> list[dict]:
        """查找危险模式（简化版静态分析）"""
        patterns = {
            'strcpy': '缓冲区溢出风险：使用 strcpy 而非 strncpy',
            'sprintf': '格式化字符串漏洞风险：使用 sprintf 而非 snprintf',
            'gets': '高危：gets 函数已被废弃，使用 fgets 替代',
            'malloc': '内存分配：检查是否有对应的 free',
            'eval(': '代码注入风险：使用 eval 执行任意代码',
            'exec(': '代码注入风险：使用 exec 执行任意命令',
            '__import__': '动态导入：可能存在不安全的模块加载',
        }

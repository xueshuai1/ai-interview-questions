...
**关键区别：** P(y|x) 路径是"看题答题"的定向优化，P(y) 路径是"自由生成 + 负样本剪枝"的泛化优化。两者梯度对齐，最终汇聚为更强的推理能力。
...
这个流程展示了 NSR-PreRL 如何形成一个自洽的训练闭环：模型生成 → 识别错误 → 负奖励抑制 → 剪枝更新 → 再生成，循环迭代直到错误路径被有效剪枝。
...
### Python 实现：双空间 RL 完整骨架

以下是一个完整的 DSRL（双空间 RL）实现骨架，展示两个空间如何协同工作：
...
这个实现骨架展示了 DSRL 的完整流程：
1. **阶段 1**：在预训练空间 P(y) 上用负样本进行剪枝，模型学会"不走错路"
2. **参考模型更新**：将剪枝后的策略作为新的参考点
3. **阶段 2**：在条件空间 P(y|x) 上做精细优化，模型学会"走对路"

先剪枝再优化的策略，就像园艺中先修枝再施肥——避免把资源浪费在错误路径上。

...
`,
  ...
  },
  {
    ...
    tagColor: "bg-yellow-500/10 text-yellow-300",
    coverImage: "/images/news/performance.jpg",
    title: "SpecGuard：验证感知的推测解码框架——用模型内部信号进行步骤级验证，准确率提升 3.6% 同时延迟降低 11%",
    summary: "SpecGuard 提出验证感知的推测解码，在每步采样多个候选，使用注意力 grounding 分数和 log-probability 分数的轻量级集成进行步骤级验证，选择性分配计算。相比推测解码和奖励引导推测解码，准确率提升 3.6%，延迟降低约 11%，无需外部奖励模型。（arXiv:2604.15244）",
    content: `## SpecGuard：无需外部奖励模型的验证感知推测解码

2026 年 4 月 16 日，arXiv 发表论文（arXiv:2604.15244）。

**核心问题：**
- 推测解码（SD）以 token 为中心推进，错误步骤会传播
- 现有方法用外部奖励模型缓解，但增加延迟和计算开销
...
**双信号验证集成：**
1. **注意力 Grounding 分数**：测量对输入和已接受步骤的归因
2. **Log-probability 分数**：捕获 token 级置信度
...
- 对多步推理场景特别有价值
`,
    date: "2026-04-17 10:18",
    source: "arXiv 2604.15244",
    sourceUrl: "https://arxiv.org/abs/2604.15244",
    href: "/news/news-269",
  },
  {
    id: "news-270",
    tag: "空间智能",
    tagColor: "bg-indigo-500/10 text-indigo-300",
    coverImage: "/images/news/spatial.jpg",
    title: "VRU 视角旋转理解：LLM/VLM 在无视觉输入下如何理解空间旋转？ACL 2026 揭示最终层幻觉机制",
    summary: "ACL 2026 Main 论文研究 LLM 和 VLM 在纯文本输入下理解视角旋转的能力。发现模型在 VRU 数据集上表现很差（人类 100% 准确率），层间探测分析显示模型在隐藏状态中编码了视角信息，但最终层无法将视角位置与对应观察绑定，产生幻觉。选择性微调关键注意力头可提升 VRU 性能而不导致灾难性遗忘。（arXiv:2604.15294）",
    content: `## VRU：LLM 空间智能的根本局限

2026 年 4 月 16 日，ACL 2026 Main 论文（arXiv:2604.15294）。

**核心问题：**
- 空间智能受到越来越多关注
- 但在没有视觉信息时，纯语言智能是否足以赋予模型空间能力？

**研究设计：**
- **视角旋转理解（VRU）**任务
- LLM/VLM 根据文本描述推断最终视角和对应观察
- 人类可轻松达到 **100% 准确率**

**关键发现：**
1. **模型表现很差**：LLM 和 VLM 都无法有效完成 VRU 任务
2. **视角信息确实被编码**：隐藏状态中包含视角信息
3. **最终层幻觉**：模型无法将视角位置与对应观察绑定
4. **选择性微调有效**：微调因果干预识别的关键注意力头可提升性能
5. **无灾难性遗忘**：通用能力得以保留

**行业意义：**
- 揭示了 LLM 空间智能的根本局限
- 编码 ≠ 理解：信息在隐藏状态中但不被正确使用
- 为空间智能改进提供了明确方向
- 数据集开源：https://github.com/Young-Zhen/VRU_Interpret
`,
    date: "2026-04-17 10:20",
    source: "arXiv 2604.15294",
  ...
  },
  {
    ...
    tagColor: "bg-violet-500/10 text-violet-300",
    coverImage: "/images/news/eval.jpg",
    title: "LLM 最短路径泛化研究：空间迁移强但长度扩展失败——RL 改善稳定性但不扩展能力边界",
    summary: "提出基于最短路径规划的可控合成环境，分离数据覆盖、训练范式和推理策略对系统泛化的影响。发现模型在空间迁移（未见地图）上表现强，但因递归不稳定性在长度扩展（更长视野）上持续失败。数据覆盖设定能力上限，RL 改善训练稳定性但不扩展上限，推理时扩展增强性能但无法挽救长度扩展失败。（arXiv:2604.15306）",
    content: `## LLM 系统泛化的根本局限：最短路径案例研究

2026 年 4 月 16 日，arXiv 发表论文（arXiv:2604.15306）。

**核心问题：**
- 语言模型能否系统性泛化仍在激烈争论
- 实证性能受训练数据、训练范式和推理策略共同影响
...
**学习管线各阶段影响：**
1. **数据覆盖**：设定能力上限
2. **强化学习**：改善训练稳定性，但不扩展上限
3. **推理时扩展**：增强性能，但无法挽救长度扩展失败
...
- 为模型能力边界提供了实证理解
`,
    date: "2026-04-17 10:22",
    source: "arXiv 2604.15306",
    sourceUrl: "https://arxiv.org/abs/2604.15306",
    href: "/news/news-271",
  },
];

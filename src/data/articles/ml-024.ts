...
**联邦学习的核心思想**可以用一句话概括：**"把代码送到数据那里，而不是把数据送到代码那里。"** 一个全局模型被分发给多个参与方（客户端），每个参与方在本地数据上训练该模型，只将模型更新（权重或梯度）传回中央服务器进行聚合，而原始数据始终保留在本地。
...
但联邦学习也面临独特挑战：**通信瓶颈**——模型更新在网络上传输的开销远大于集中式训练的本地内存拷贝；**数据异构性（Non-IID）**——各客户端的数据分布往往高度不均匀；**系统异构性**——不同设备的计算能力和网络带宽差异巨大。`,
            mermaid: `graph LR
    A["中央服务器<br/>Global Model"] -->|"下发模型"| B["客户端 A<br/>数据 A"]
    A -->|"下发模型"| C["客户端 B<br/>数据 B"]
    A -->|"下发模型"| D["客户端 C<br/>数据 C"]
    B -->|"上传梯度"| A
    C -->|"上传梯度"| A
    D -->|"上传梯度"| A
...
**算法流程**非常直观：服务器选择一个客户端子集，将当前全局模型发送给它们；每个客户端在本地数据上运行若干轮 SGD（随机梯度下降）；客户端将更新后的模型权重传回服务器；服务器对所有客户端的权重进行加权平均（按数据量加权），得到新的全局模型。

FedAvg 的关键创新在于**本地多轮更新（Local Epochs）**。在传统的分布式 SGD 中，每个客户端只计算一次梯度就上传，通信开销极大。FedAvg 允许客户端在本地执行多个训练步骤后再上传，大幅减少了通信轮数。

但 FedAvg 在**非独立同分布（Non-IID）数据**上表现不佳。当不同客户端的数据分布差异较大时（比如医院 A 主要处理肺炎病例，医院 B 主要处理骨折病例），全局模型的加权平均会导致各客户端的本地更新相互抵消，收敛速度显著下降。这就是后续 FedProx、FedMA 等算法要解决的问题。`,
            code: [
                {
                    lang: "python",
...
**FedMA**（Federated Matching Averaging）走的是另一条路：它认识到不同客户端的神经元可能学习到了不同的特征，简单的权重平均没有意义。FedMA 通过**层对层的匹配（Layer-wise Matching）**，先对齐不同客户端的神经元，再进行聚合。这在 CNN 和 LSTM 上效果显著。
...
**MOON**（Model-Contrastive Federated Learning）则引入了对比学习的思想：它鼓励本地模型学习到的表示与全局模型学习到的表示保持一致，同时保留本地数据的特异性。这种方法在图像分类和 NLP 任务上都有出色的表现。`,
        ...
        },
        {
            ...
            body: `联邦学习虽然不把原始数据传出本地，但模型更新（梯度）本身就可能泄露信息。研究表明，攻击者可以通过分析梯度反推出训练数据中的敏感内容（如姓名、身份证号、医疗记录）。这就是为什么**差分隐私（Differential Privacy, DP）**成为联邦学习不可或缺的补充。

**差分隐私的核心机制**是在模型更新中添加可控的随机噪声。具体来说，每个客户端在上传梯度之前，先对梯度进行裁剪（Clipping），限制单个样本对梯度的最大影响（L2 范数裁剪），然后添加高斯噪声。噪声的幅度由隐私预算 ε（epsilon）控制：ε 越小，隐私保护越强，但模型精度下降越多。

**DP-FedAvg** 是联邦学习中最常用的差分隐私方案。它与标准 FedAvg 的区别仅在于客户端上传梯度时添加了噪声。关键在于**隐私预算的累积管理**——每一轮训练都会消耗一部分隐私预算，需要通过矩会计（Moments Accountant）或 RDP（Rényi Differential Privacy）精确追踪总消耗量。

2024-2025 年的一个重要趋势是**联邦学习 + LoRA**的结合。由于 LoRA 只训练低秩适配器（通常只有模型参数的 0.1%-1%），需要传输的数据量大幅减少，同时噪声对低秩空间的影响也更可控。NeurIPS 2025 的 FedASK 工作进一步将 DP-Sketching 引入联邦 LoRA，在保护隐私的同时保持了高质量的联合微调效果。`,
            code: [
                {
                    lang: "python",
...
class DPClient:
    ...
    def get_privacy_budget(
        ...
        epsilon = np.sqrt(
            2 * n_rounds * np.log(1/delta) * (q**2) / (sigma**2)
        )
        return epsilon
...
**模型压缩**是最直接的优化手段。主要包括三种技术：**量化（Quantization）**——将 32 位浮点权重压缩到 8 位甚至更低，通信量减少 4 倍；**稀疏化（Sparsification）**——只传输变化最大的梯度分量（如 Top-k 稀疏化），其余置零；**低秩近似**——用低秩矩阵近似权重更新，只传输分解后的因子。

**异步联邦学习**打破了同步等待的限制。在标准 FedAvg 中，服务器必须等待所有选定客户端完成训练才能聚合。在异构环境中，慢速客户端（"掉队者"，Stragglers）会严重拖慢整体进度。异步方案允许服务器在收到足够多的客户端更新后立即聚合，不等待所有人完成。
...
**FATE（Federated AI Technology Enabler）** 是微众银行开源的企业级联邦学习平台。它的特点是支持**横向联邦**（样本重叠少、特征空间相同）和**纵向联邦**（样本重叠多、特征空间不同）两种模式。纵向联邦特别适合金融风控场景——多家银行可以联合训练风控模型而不共享客户数据。

**PySyft** 是 OpenMined 开发的隐私计算框架，结合了联邦学习和安全多方计算（MPC）。它的独特优势是**加密计算**——即使在聚合阶段，服务器也无法看到单个客户端的更新。

部署联邦学习的**最佳实践**：(1) 从小规模 PoC 开始，先验证模型质量和通信开销；(2) 使用 Flower 的仿真模式（Simulation Mode）在单机上模拟多客户端行为；(3) 在生产环境中先采用 FedProx 而非 FedAvg，因为真实数据几乎总是 Non-IID 的；(4) 始终启用差分隐私——即使法规不要求，它也是防御梯度反演攻击的有效手段。`,
            code: [
                {
                    lang: "python",
...
**联邦大模型微调**是当前的热点。LLM 的微调需要大量领域特定数据，而这些数据往往分布在不同的组织手中。联邦 LoRA（Federated LoRA）允许多个组织联合微调一个 LLM，每个组织只训练自己的低秩适配器，然后通过联邦学习聚合。这不仅保护了数据隐私，还大幅降低了通信开销——因为只需要传输 LoRA 适配器（通常只有几 MB），而不是整个模型（几十 GB）。

**联邦学习与安全计算的融合**是另一个重要方向。单纯的安全聚合（Secure Aggregation）只能防止服务器看到单个客户端的更新，但无法防御恶意客户端的投毒攻击（Poisoning）。联邦平均（Federated Averaging）本身对投毒也很敏感。未来的趋势是将联邦学习与同态加密（HE）、安全多方计算（MPC）、零知识证明（ZKP）深度融合，构建多层防御体系。

**去中心化联邦学习**正在探索中。传统的联邦学习依赖一个可信的中央服务器进行聚合。去中心化方案（如基于区块链的联邦学习）消除了对中央服务器的信任依赖，每个参与方既是客户端也是聚合节点。这在加密货币和 Web3 场景中有独特的价值。

**联邦学习的标准化**也在推进。2025 年，IEEE P3652.1 工作组发布了联邦学习的首个国际标准草案，涵盖了术语、架构、安全要求和评测基准。标准化将加速联邦学习在医疗、金融等高监管行业的落地。`,
            table: {

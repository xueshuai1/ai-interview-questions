...
graph TB
    subgraph "研究者输入"
        A[program.md<br/>研究方向描述]
    end
    
    subgraph "AI Agent 循环"
        B[AI Agent<br/>读取 program.md]
        C[修改代码<br/>src/*.py]
        D[启动训练<br/>5 分钟短训练]
        E[评估结果<br/>验证集指标]
        F{是否改进?}
    ...
    subgraph "nanochat 框架"
        G[nanochat<br/>轻量 LLM 训练框架]
        H[单 GPU 高效训练]
    end
    
    subgraph "输出"
        I[保留有效改动<br/>commit & push]
        J[丢弃无效改动<br/>git reset]
        K[生成实验报告]
    end
...
graph LR
    subgraph "数据层"
        A[Shakespeare 数据集]
        B[Tokenizer<br/>字符级/子词级]
    end
    
    subgraph "模型层"
        C[NanoGPT<br/>Transformer]
        D[Embedding]

...
MoE 的核心价值在于**解耦了模型容量和计算成本**。你可以无限增加专家数量来提升模型的知识容量和表达能力，而不必承担对应的计算开销。这种"参数多但算得少"的特性，使得 MoE 成为当前最具性价比的大模型架构方案。`,
            mermaid: `graph TD
    ...
    style E fill:#E0E0E0`,
        ...
        },
        {
            ...
            body: `MoE 层由三个核心组件构成：**专家网络（Expert Networks）**、**门控网络（Gating Network/Router）**和**组合函数（Combination Function）**。

**专家网络**是一组并行的前馈神经网络（通常是 FFN/MLP）。每个专家专注于学习输入空间中某个子区域的知识。在 Transformer 架构中，MoE 层通常替换每个 Transformer 块中的标准 FFN 层。如果有 E 个专家，每个专家的参数量为 P_expert，则 MoE 层的总参数量为 E × P_expert。

**门控网络**是 MoE 的大脑。对于每个输入 token x，门控网络计算一个路由权重向量 g(x) ∈ R^E，决定将 token 分配给哪些专家。最常用的路由方式是 Top-K 路由：选择 g(x) 中权重最大的 K 个专家，其余专家的权重设为零。
...
print(f"每次激活参数量: {768*3072*2 + 768*8:,} (仅 2 个专家 + 门控)")`,
        ...
        },
        {
            ...
            body: `路由策略是 MoE 架构中最关键的设计选择。一个糟糕的路由器会导致两个严重问题：**专家负载不均**和**训练不稳定**。

**负载均衡问题**是最常见的 MoE 陷阱。如果路由器倾向于将大多数 token 分配给少数几个专家，这些专家就会过载，而其他专家则闲置。更严重的是，这会导致被偏爱的专家过度训练，进一步强化了路由偏好，形成正反馈循环——最终只有 1-2 个专家在实际工作，其他专家形同虚设。
...
**DeepSeek 的无辅助损失负载均衡**是 2026 年的重要创新。DeepSeek-V3 提出了一种不依赖辅助损失的负载均衡策略：在路由 logits 中直接添加可学习的偏置项（bias），通过梯度下降自动调整偏置值来实现负载均衡。这种方法避免了辅助损失对主任务梯度的干扰，训练更稳定。

**序列级路由 vs Token 级路由**是另一个重要选择。Token 级路由为每个 token 独立选择专家，灵活性高但可能导致同一序列的 token 分散到太多专家，增加通信开销。序列级路由将整个序列路由到相同的专家子集，减少了专家切换开销但降低了灵活性。DeepSeek-V3 采用了混合策略：在 attention 层使用序列级路由，在 FFN 层使用 token 级路由。`,
            table: {
        ...
        {
            title: "4. 专家容量与训练稳定性",
            body: `MoE 训练中，每个专家能处理的 token 数量是有限的，这个上限称为**专家容量（Expert Capacity）**。如果某个专家接收的 token 超过容量，多余的 token 会被"溢出"（drop）——即不经过该专家处理，直接跳过。

专家容量的设置是一个微妙的平衡。**容量太小**会导致大量 token 溢出，信息丢失，模型性能下降。**容量太大**则浪费计算资源，因为需要为最坏情况预留足够的计算空间，而大多数时候专家的实际负载远低于容量。
...
**专家缓存策略**是推理优化的核心。在 GPU 内存有限的情况下，无法同时加载所有专家。常见策略包括：
...
**投机解码（Speculative Decoding）与 MoE 的结合**是 2026 年的重要进展。投机解码使用一个小模型快速生成候选 token，然后用大模型验证。在 MoE 架构中，可以用一个小型稠密模型作为草稿模型，用 MoE 大模型作为验证模型。由于 MoE 每次只激活部分专家，验证成本远低于推理一个同等参数量的稠密模型，使得投机解码的加速效果更加显著。
...
**MoE + 多模态**是下一个前沿。在多模态模型中，不同专家可以专门处理不同模态：视觉专家处理图像，语言专家处理文本，音频专家处理声音。路由器根据输入模态自动选择合适的专家组合。这种设计比单一的多模态融合层更加灵活和高效。Google 的 Gemini 和 OpenAI 的 GPT-4o 都被认为采用了类似的多模态 MoE 架构。

**MoE + Agent**是另一个激动人心的方向。在 AI Agent 系统中，不同专家可以对应不同的工具或技能：代码专家负责编程任务，数学专家负责推理计算，检索专家负责信息查询。路由器根据用户请求的类型自动选择最合适的专家。这与人类专家协作的方式高度一致——遇到编程问题找程序员，遇到法律问题找律师。

**动态专家增长**是 MoE 架构的长期愿景。当前的 MoE 模型专家数量是固定的，但未来的模型可能能够动态添加新专家——当遇到新知识领域时，自动"雇佣"新的专家来学习这个领域。这将使模型具备持续学习的能力，而不会干扰已有的知识（因为新知识存储在新专家中）。

...
这些观察精准命中了每个使用 LLM 编码工具（Claude Code、Cursor、Codex、GitHub Copilot）的开发者都会遇到的痛点。而 andrej-karpathy-skills 项目将这些观察提炼为**四大可执行原则**，并通过一个简单的 CLAUDE.md 文件让所有 Claude Code 用户受益。
...
这个原则的核心是将**命令式指令**（"添加验证"）转化为**声明式目标**（"为无效输入编写测试，然后让它们通过"）。
...
- ✅ PR 干净、精简——没有多余的改动`,
    ...
    {
      title: "9. 总结与展望",
      body: `Karpathy 四大原则之所以能在短短一周内获得近 30,000 星，核心原因在于它解决了一个**每个 LLM 编码用户都会遇到的实际问题**。
...
**关键 takeaway：** 改善 LLM 编码质量不一定需要复杂的工具链或昂贵的微调。有时候，**正确的思维框架**比任何技术方案都有效。四大原则本质上是一套"思维习惯"——它们不改变 LLM 的能力，而是改变 LLM 使用能力的方式。

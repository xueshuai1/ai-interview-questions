...
Cursor 的核心优势在于**上下文感知的代码理解**。它自动索引整个代码库，构建符号表（函数、类、变量的定义和使用关系），让 AI 在生成代码时能准确理解跨文件依赖。当你让 Cursor "修改用户认证逻辑"时，它不仅能找到 auth.ts 文件，还能识别所有引用认证函数的地方，并同步更新。
...
这种多 Agent 协作模式的优势在于**分工和并行**。一个复杂的重构任务（如"将单体应用拆分为微服务"），单 Agent 可能需要数小时，而多 Agent 并行可以将时间缩短到分钟级。
...
另一个重要实践是**代码解释**——要求 AI 在生成代码后，用自然语言解释每段代码的作用、为什么选择这个方案、有什么潜在风险。这不仅帮助人类审查，也让 AI 自己"反思"生成质量。`,
            code: [
                {
                    lang: "typescript",
...
2026 年的一个重要趋势是**AI 代码的供应链管理**。随着 AI 生成的代码在代码库中的比例不断增加（有些项目已经超过 50%），如何追踪哪些代码是人类写的、哪些是 AI 生成的、AI 生成了哪些依赖——这些问题正在成为软件工程的新课题。一些工具已经开始支持在 Git commit 中记录 AI 生成信息，方便审计和追溯。`,
            code: [
                {
                    lang: "text",
                    code: `# AI 编程实战工作流

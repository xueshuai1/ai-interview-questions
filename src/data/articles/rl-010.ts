...
折扣因子 gamma 决定了未来奖励的重要性。gamma 接近 0 时，智能体只关心即时奖励（短视）；gamma 接近 1 时，智能体同等看待遥远的未来奖励（长远）。`,
            mermaid: `graph LR
    A["状态空间 S<br/>所有可能状态"] --> B["动作空间 A<br/>可执行的动作"]
    B --> C["转移概率 P<br/>环境动态特性"]
    C --> D["奖励函数 R<br/>即时反馈"]
    D --> E["折扣因子 gamma<br/>未来奖励权重"]

    A -.->|马尔可夫性| F["P(s'|s,a)<br/>不依赖历史"]
...
卷积编码器将原始像素输入（如 Atari 游戏的 210×160 RGB 图像）编码为低维状态表示。这是 DQN 能直接从像素学习的关键。`,
            mermaid: `graph TD
    A["原始像素输入<br/>210×160×3"] --> B["预处理<br/>灰度+裁剪+下采样"]
    B --> C["卷积层 1<br/>32 filters, 8×8"]
    C --> D["卷积层 2<br/>64 filters, 4×4"]
    D --> E["卷积层 3<br/>64 filters, 3×3"]
    E --> F["Flatten"]
    F --> G["全连接层 512"]
    G --> H["Q 值输出层<br/>N 个动作的 Q 值"]
...
PPO-Clip 的完整目标函数为：L^CLIP(theta) = E[min(ratio · A, clip(ratio) · A)]，其中 A 是优势函数。这个目标函数同时考虑了未裁剪和裁剪的梯度，确保更新既有效又安全。`,
            mermaid: `graph TD
    A["收集轨迹<br/>使用旧策略 pi_old"] --> B["计算优势函数 A(s,a)"]

    ...
    content: [
        {
            ...
            body: `在大语言模型的发展过程中，研究者发现两种极端策略都存在明显的局限性。纯推理策略（如 Chain-of-Thought）让模型在内部进行多步思考，但不与外部环境交互。这种方式在数学推理、逻辑推导等封闭任务上表现优异，但一旦涉及需要外部知识的开放任务——比如回答「2024年诺贝尔奖得主是谁」——模型只能依赖训练数据中的过期信息，无法获取最新事实。
...
"""`
    ...
    I -.->|推理充分| L["Answer 回答"]`,
        ...
        },
        {
            ...
            body: `ReAct 范式由 Yao 等人在 2022 年的论文「ReAct: Synergizing Reasoning and Acting in Language Models」中正式提出。论文的核心贡献在于揭示了一个关键现象：推理轨迹（Reasoning Traces）和行动轨迹（Action Traces）之间存在相互增强的协同效应。
...
ReAct 的主要局限性包括：Token 消耗大——每一轮 TAO 循环都需要 LLM 重新生成完整的上下文，导致 token 使用量随步数线性增长；传播错误——如果某一步的 Observation 是错误的（比如搜索返回了不相关的结果），后续的所有 Thought 和 Action 都可能基于这个错误信息做出错误决策；工具依赖——ReAct 的效果高度依赖于工具的质量，如果工具返回的信息不完整或不准确，再好的推理也无法得到正确答案；解析脆弱性——LLM 可能输出不符合预期格式的 Action，导致 Agent 无法正确解析和执行。

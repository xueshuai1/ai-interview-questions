...
> Agent 先运行了 \`npm install\`，输出了几百行依赖安装日志；然后执行 \`git status\`，列出了几百个修改文件；接着运行测试，又输出了几千行测试结果……最终，Agent 的上下文窗口被这些**工具输出（Tool Output）淹没**，导致它「忘记」了最初的任务目标，开始重复执行已经做过的操作，或者给出与当前状态不符的建议。

这就是 **上下文窗口爆炸（Context Window Explosion）** 问题——AI 编码 Agent 最常见、最影响效率的痛点。

### 问题根源：为什么工具输出会「吃掉」上下文？

传统 AI 编码 Agent 的工作流中，每次工具调用（执行命令、读取文件、运行测试等）的输出都会被**完整追加**到对话历史中：
...
**105K tokens**——这已经接近甚至超过了大多数模型的上下文窗口上限（Claude Sonnet 4.5 为 200K，GPT-4.1 为 1M 但成本极高）。而且这还是**一个简单任务**的情况。
...
context-mode（mksglu/context-mode，9,980⭐）是这一流派的代表。它通过在工具输出管道中插入一个**压缩层**，将冗长的输出转换为精简摘要：
...
- 不适合一次性任务`,
    ...
    },
    {
      ...
      body: `context-mode 的核心创新在于它的**分层压缩管道（Layered Compression Pipeline）**。让我们深入理解它是如何实现 98% 上下文缩减的。
...
未来的上下文优化系统将不再采用单一的压缩策略，而是根据**任务类型、上下文位置、信息重要性**动态选择压缩方式：
...
像 claude-mem 这样的项目正在探索将上下文记忆**向量化存储**，实现：
...
context-mode 等工具正在从**后处理压缩**向**端到端优化**演进：

...
选择检测方法的关键考量：检测灵敏度（能否发现微小但重要的变化）、计算效率（能否实时运行）、可解释性（能否定位漂移的具体特征）、以及假阳性率（过于敏感会导致告警疲劳）。`,
    ...
    F -->|是 - 低延迟| G["PSI + 滑动窗口<br>O(N) 计算"]
    F -->|否 - 离线批处理| H["MMD + 全量数据<br>更高灵敏度"]
...
分析和告警层是整个平台的大脑。它消费存储层的数据，执行漂移检测算法、计算性能指标、判断是否触发告警。告警需要分级：信息级（记录但不打扰）、警告级（通知值班人员）、严重级（立即通知并可能触发自动回滚）。`,
        mermaid: `graph LR
    A["推理服务"] -->|记录日志| B["数据采集层"]
    B -->|实时流| C["消息队列<br>Kafka / PubSub"]
    C --> D["流处理引擎<br>Flink / Spark"]
    D --> E["时序数据库<br>Prometheus"]
    D --> F["数据仓库<br>Snowflake"]
    
    E --> G["实时监控面板<br>Grafana"]
    F --> H["离线分析<br>Jupyter"]
    
    D --> I["漂移检测引擎"]
    I -->|告警规则| J["告警系统<br>PagerDuty / 飞书"]
    J --> K["自动触发重训练"]`,
        table: {
...
告警的降噪（Deduplication）同样重要。同一根因可能触发多个告警，需要聚合和去重。比如数据漂移和预测分布漂移可能源于同一个上游数据管道变更，应该合并为一条告警。`,
    ...
    B -->|轻微 PSI < 0.15| C["P3 信息告警<br>记录到仪表板"]
    B -->|中等 0.15-0.25| D["P2 警告告警<br>通知值班人员"]
    B -->|严重 PSI > 0.25| E["P1 严重告警<br>立即通知 + 自动响应"]

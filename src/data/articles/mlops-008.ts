...
但量化不是简单的"降低精度"。模型权重中不同参数的重要性差异巨大——有些权重对输出影响很大（异常值，Outliers），有些几乎无关紧要。好的量化方法能够**智能地识别哪些参数需要高精度、哪些可以压缩**，从而在精度损失最小化的同时实现最大的压缩比。
...
2024-2025 年是 LLM 量化技术爆发的年份。GPTQ、AWQ、SmoothQuant、bitsandbytes 的 NF4 格式、以及 llama.cpp 的 K-quants 体系，构成了一个完整的量化工具链。理解它们的区别和适用场景，是每个 AI 工程师的必修课。`,
            mermaid: `graph LR
    A["FP16 全精度<br>14GB (7B模型)"] -->|"INT8 量化"| B["INT8<br>7GB (~99%精度)"]
    B -->|"INT4 量化"| C["INT4 GPTQ<br>3.5GB (~95%精度)"]
    C -->|"INT2 量化"| D["INT2/1-bit<br>1.75GB (~85%精度)"]
    ...
    style D fill:#fce4ec`,
        },
        {
            ...
            body: `LLM 量化有两种主要范式：**训练后量化（Post-Training Quantization, PTQ）**和**量化感知训练（Quantization-Aware Training, QAT）**。选择哪种范式取决于你的资源、时间和精度要求。
...
print("PTQ 适合快速部署，QAT 适合追求极致精度")`
        ...
        },
        {
            ...
            body: `GPTQ（Generative Pre-trained Transformer Quantization）是 2023 年提出的 PTQ 算法，迅速成为 INT4 量化的事实标准。它的核心洞察是：LLM 的权重矩阵具有**冗余结构**，可以通过逐层的独立量化来实现接近 QAT 的精度，而不需要任何训练。

**GPTQ 的算法流程**非常优雅。对于模型中的每一层线性层（Linear Layer），GPTQ 执行以下步骤：首先，在一个校准数据集上收集该层的激活输入；然后，通过近似二阶优化（基于 Hessian 矩阵的逆）来确定最佳的量化方案；最关键的是，GPTQ 会**逐元素地量化权重**——每量化一个权重，就更新剩余权重的量化误差补偿，确保误差最小化。
...
**GPTQ 的局限**在于量化速度——对一个 70B 模型进行 GPTQ 量化需要数小时和一张高端 GPU。但对于生产部署来说，这是一次性开销，之后的推理速度和内存节省带来的回报远超量化成本。

2025 年的重要进展包括**GPTQ-Marlin**——针对 NVIDIA 张量核心的推理内核优化，使 GPTQ INT4 模型的推理速度接近 FP16 的 2-3 倍；以及**Meta's SpinQuant**——用旋转矩阵预处理权重，使 GPTQ 在极低比特（如 INT2）下的表现显著提升。`,
            code: [
                {
                    lang: "bash",
...
"`
        ...
        },
        {
            ...
            body: `AWQ（Activation-aware Weight Quantization）是另一个明星级 PTQ 算法。它与 GPTQ 的思路不同：不是通过复杂的优化来最小化量化误差，而是先**找到模型中最重要的权重**——那些对激活值影响最大的权重——然后保护它们不被过度量化。
...
在 NVIDIA H100 上，SmoothQuant INT8 模型的推理速度可以达到 FP16 的 1.3-1.5 倍；如果进一步结合 TensorRT-LLM 的 FP8 支持，加速比可以达到 1.8 倍以上。`,
            mermaid: `graph TD
    A["异常激活值"] -->|"乘以 s^(-1)"| B["平滑后的激活值<br>适合 INT8 量化"]
    C["权重矩阵"] -->|"乘以 s"| D["缩放后的权重<br>仍适合 INT8 量化"]
    
    B --> E["W8A8 推理"]
    D --> E
    E --> F["INT8 Tensor Core<br>最大推理加速"]
    ...
    style E fill:#e8f5e9`,
        },
        {
            ...
            body: `在实际部署中，你面对的不仅是算法选择，还有**量化格式**的选择。不同的格式支持不同的硬件、不同的推理框架、以及不同的压缩策略。

**GGUF**（GGML Unified Format）是 llama.cpp 的模型格式，也是本地运行 LLM 的事实标准。GGUF 支持多种量化预设（Q2_K 到 Q8_0），每种预设都是精度和速度的权衡。Q4_K_M（4-bit Medium）是最受欢迎的选择——在 7B 模型上只需约 4GB 内存，质量保留约 95%。GGUF 的最大优势是**跨平台**——它可以在 CPU、macOS（Apple Silicon）、甚至树莓派上运行。
...
**决策框架**：首先明确你的约束条件。显存预算是多少？延迟要求是什么？精度损失可接受范围多大？是否需要微调能力？这些问题的答案直接决定了你的量化方案。对于大多数场景，**从 INT8 开始**是最安全的选择——INT8 的精度损失通常不到 1%，且几乎所有推理引擎都支持。如果显存仍然不够，再考虑 INT4。
...
**监控要点**：量化模型的监控与传统模型不同。除了常规的延迟、吞吐量、错误率之外，还需要关注**量化退化（Quantization Degradation）**——模型在特定输入分布上的质量是否会随时间下降。这是因为量化模型对输入分布的变化更加敏感。如果发现量化模型在某种类型的 query 上持续表现不佳，可能需要针对该类型数据进行 QAT 重新量化。

**混合精度策略**是 2025-2026 年的重要趋势：不对整个模型使用统一的量化精度，而是对不同的层使用不同的精度。例如，嵌入层和最后一层使用 FP16，中间层使用 INT4。这种策略在精度和内存之间取得了更精细的平衡。vLLM 和 TensorRT-LLM 都已经支持混合精度推理。

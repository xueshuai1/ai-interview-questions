...
**核心设计理念：**
LangGraph 将多智能体协作建模为**有向图（Directed Graph）**，每个节点是一个 Agent 或工具，边定义了执行流。支持循环图，这意味着 Agent 可以迭代式地改进输出。
...
def code_agent(state: AgentState) -> AgentState:
    # 执行代码生成
    result = generate_code(state["task"])
    return {"code_result": result}

def security_agent(state: AgentState) -> AgentState:
    # 执行安全审查
    review = review_security(state["code_result"])
...
**适用场景：** 需要精细控制执行流、支持中断和恢复的生产级应用。`,
        ...
        },
        {
            ...
            body: `**CrewAI** 采用了一种更直觉化的方式——你为每个 Agent 定义**角色（Role）**、**目标（Goal）**和**背景（Backstory）**，然后将它们组织成一个 Crew。
...
**核心设计理念：**
ADK 采用 **YAML 配置 + 可视化编排**的方式定义多 Agent 系统，降低了开发门槛。
...
**适用场景：** 需要快速原型开发和可视化调试的多 Agent 应用。`
        ...
        {
            title: "6. 2026 年框架选型指南",
            body: `面对如此多的多 Agent 框架，如何选择？以下是基于场景的选型建议：

**如果你需要精细的执行流控制：**
→ 选择 **LangGraph**。状态机模型让你可以精确控制每一步的执行和分支。

**如果你需要模拟人类团队协作：**
→ 选择 **CrewAI**。角色驱动的方式最符合人类直觉。

**如果你已经在 OpenAI 生态中：**
→ 选择 **OpenAI Agent Teams**。原生集成、Guardrails 和 Handoffs 机制开箱即用。

**如果你重视安全和推理能力：**
→ 选择 **Anthropic Agent Teams**。Claude 的推理能力和安全对齐是最大优势。

**如果你需要快速原型和可视化：**
→ 选择 **Google ADK**。YAML 配置和可视化编排降低了学习曲线。

**如果你需要多 Agent 并行开发环境：**
→ 选择 **JetBrains Air**。专为 Agent 驱动的开发流程设计。

**关键决策因素：**
1. 现有基础设施（LLM 提供商、云平台）
2. 团队技术栈（Python/TypeScript/其他）
3. 安全合规要求
4. 是否需要人机协作（中断/恢复）
5. 部署规模（开发环境 vs 生产环境）`
        },

...
LLM 是一次性输入输出的静态评估。Agent 则是**多轮交互的动态过程**：它接收任务 → 规划 → 调用工具 → 观察结果 → 调整策略 → 再调用工具 → ... → 最终完成任务。评估需要考虑整条交互链的质量，而不仅仅是最终答案。
...
AISafetyBenchExplorer 论文发布后，行业正在推动建立一个**统一的 Agent 评估框架**。预计 2026 下半年会出现：

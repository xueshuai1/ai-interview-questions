    ...
    content: [
        {
            ...
            body: `扩散模型（Diffusion Models）在过去三年统治了生成式 AI 领域。从 DALL-E 2 到 Stable Diffusion，从 Imagen 到 Midjourney，几乎每一个令人惊叹的 AI 生成作品背后都有扩散模型的影子。但到了 2025-2026 年，扩散模型的**固有局限性**变得越来越明显，推动了新一代生成范式的诞生。

**采样速度慢**是扩散模型最直接的痛点。标准的 DDPM 需要 1000 步迭代去噪，即使使用 DDIM 加速到 50 步，每一步仍然需要完整的前向传播。这意味着生成一张高质量图像可能需要数百毫秒到数秒——对于实时应用来说太慢了。虽然 LCM（Latent Consistency Model）和 SDXL Turbo 等技术将步数压缩到了 1-4 步，但这是通过额外的蒸馏训练实现的，增加了复杂性和训练成本。

**训练-推理不一致**是更深层的问题。扩散模型在训练时学习的是噪声预测（预测每一步添加的噪声），但推理时执行的是逐步去噪采样。训练目标（噪声预测的 MSE）和推理目标（从噪声生成数据）之间没有直接的数学对应关系，这导致训练过程不够高效。

**理论分析困难**也是扩散模型的短板。虽然扩散模型的经验效果出色，但其训练动力学的理论分析相对薄弱。我们很难精确理解模型在训练过程中学到了什么，也很难从理论上保证训练收敛到最优解。

**Flow Matching** 正是在这些背景下诞生的。它将生成过程建模为**常微分方程（ODE）的学习**，直接学习从噪声分布到数据分布的变换速度场。这种方法在训练效率、采样速度和理论可分析性方面都有显著优势。2025 年，Stability AI 在 Stable Diffusion 3 中采用了 Rectified Flow（整流模型），Black Forest Labs 的 FLUX 系列也基于 Flow Matching 架构，而 OpenAI 的 Sora 视频生成系统同样使用了类似的连续变换方法。`,
            table: {
    ...
    E --> H["Sora (连续变换)"]`,
        ...
        },
        {
            ...
            body: `Flow Matching 的核心思想可以用一句话概括：**学习一个速度场 v(x, t)，使得从简单分布（噪声）出发沿该速度场流动的粒子，最终到达数据分布**。
...
这个 ODE 定义了一个确定性映射：给定初始点 x(0) 和速度场 v，通过数值积分（如 Euler 方法、Runge-Kutta 方法）可以唯一确定 x(1)。这个映射是一个**微分同胚**——它既是可逆的又是光滑的，这意味着我们不仅可以从噪声生成数据（正向积分），还可以从数据反推到噪声（反向积分）。

但问题是：我们不知道真实的速度场 v 是什么。**Conditional Flow Matching (CFM)** 提供了解决方案。其核心洞察是：虽然我们不知道从 p_0 到 p_1 的直接变换，但我们可以定义一族**条件概率路径** p_t(x | x_1)，其中 x_1 是从数据分布中采样的一个具体数据点。对于每对 (x_0, x_1)，我们构造一条从 x_0 到 x_1 的条件轨迹，并计算其对应的条件速度场 u_t(x | x_1)。

然后，Flow Matching 的训练目标就是让神经网络 v_θ(x, t) 拟合这些条件速度场：

L(θ) = E_{t, x_1, x_0}[||v_θ(x_t, t) - u_t(x_t | x_1)||²]

其中 x_t 是条件路径上 t 时刻的点。这个损失函数形式上与扩散模型的噪声预测 MSE 损失非常相似，但有着本质不同：Flow Matching 直接学习速度场（即数据的变化方向），而扩散模型学习的是噪声（即需要移除的扰动）。
...
在标准的 Flow Matching 中，条件路径可以是任意的曲线。但如果我们选择**线性插值**作为条件路径（即 x_t = (1-t)·x_0 + t·x_1），那么条件速度场就是常数 u_t(x | x_1) = x_1 - x_0。这意味着对于每对 (x_0, x_1)，粒子沿直线从 x_0 运动到 x_1，速度恒定。

但这里有一个关键问题：虽然单个粒子的条件路径是直线，但**所有粒子的整体流（marginal flow）可能不是直线**。这是因为不同的粒子有不同的目标 x_1，它们的速度场在空间中可能相互交叉或弯曲。

整流模型的解决方案是**迭代整流（Iterative Rectification）**：
...
神奇的事情发生了：经过几轮迭代后，学习到的速度场越来越接近常向量场，ODE 轨迹越来越接近直线。当轨迹完全变成直线时，**只需要一步 Euler 积分**就能从噪声精确生成数据——这就是整流模型"Rectified"的含义。

从最优传输（Optimal Transport）的角度理解，整流模型实质上是在学习数据分布和噪声分布之间的**Brenier 映射**——这是最优传输理论中保证存在的唯一最优传输映射。Brenier 映射的一个重要性质是它是某个凸函数的梯度，这意味着传输路径是"直的"（在某种几何意义下）。
...
**FLUX（Black Forest Labs, 2025）** 是 SD3 团队的后续作品，将 Flow Matching 推向了新的高度。FLUX 采用了一个纯 Transformer 架构（没有卷积层），使用 Flow Matching 作为训练目标，并引入了 Flow Rectification 技术。FLUX 的最大特点是**流匹配 + 注意力机制的深度整合**：在 Transformer 的每一层中，注意力模块处理全局信息，MLP 模块同时作为速度场预测器。这种设计使得 FLUX 在图像细节和结构一致性上达到了新的高度。FLUX.1 系列包含三个版本：FLUX.1 [pro]（闭源旗舰）、FLUX.1 [dev]（开源、非商用）和 FLUX.1 [schnell]（开源、快速，仅需 4 步采样）。

**Sora（OpenAI, 2024-2025）** 虽然 OpenAI 没有完全公开技术细节，但根据论文 "Video Generation Models as World Simulators" 中的描述，Sora 的核心是**时空 Patch 的连续变换**。它将视频分解为时空 Patch（同时包含空间和时间维度），然后在 Patch 空间上学习一个 Flow Matching 模型。与图像生成不同，视频生成需要建模时间维度上的动态变化。Flow Matching 的 ODE 框架天然适合这种任务——ODE 的时间变量 t 可以同时编码生成进度和视频帧的时间位置。Sora 能够生成长达 60 秒的高清视频，这在之前的扩散模型框架下是极其困难的。`,
            code: [
                {
                    lang: "python",

...
Transformer 的核心是**自注意力机制（Self-Attention）**。与 Seq2Seq 中的交叉注意力不同，自注意力让序列中的每个位置都能关注到序列中的所有其他位置（包括自己）。计算过程可以用一个简洁的公式概括：Attention(Q, K, V) = Softmax(QKᵀ / √dₖ) · V。
...
Transformer 引入了**多头注意力（Multi-Head Attention）**：不是只计算一组 Q、K、V，而是计算多组（通常 8 或 12 组），每组学习不同的'关注模式'。比如，一个头可能关注语法关系（主谓一致），另一个头可能关注语义关系（代词指代），还有一个头可能关注位置关系。多个头的输出被拼接后再经过一个线性变换，得到最终的多头注意力输出。

...
视频理解的主要任务分支包括：**动作识别**（整个视频一个标签）、**时序动作定位**（Temporal Action Localization，检测动作的起止时间）、**时空动作检测**（Spatio-Temporal Action Detection，同时定位动作的空间边界框和时间范围）以及**视频问答**（Video QA，结合语言理解回答关于视频内容的问题）。
...
print(f"3D kernel shape: {conv3d.weight.shape}")  # (128, 64, 3, 3, 3)`,
    ...
    },
    {
      ...
      body: `Two-Stream 网络是视频理解领域最具影响力的架构之一，由 Simonyan 和 Zisserman 在 2014 年提出。其核心洞察是：视频中的信息可以分为两类——**空间信息**（场景、物体外观）和**时间信息**（运动模式、动态变化）——这两类信息适合用不同的网络分支独立处理。
...
两个通路在多个阶段通过**横向连接（Lateral Connections）**进行特征融合：Fast 通路的特征经过 3D 卷积变换后注入到 Slow 通路中。这种设计使得网络能够同时获得高质量的空间表征和精细的时序动态，在多个视频理解基准上取得了优异性能。`,
      code: [
        {
          lang: "python",
          code: `import torch
...
在实际部署中，还需要考虑**多段采样（Multi-crop）**策略：对视频进行多次不同起点的采样和不同空间裁剪，将多次推理的结果取平均，可以显著提升准确率。这是竞赛和论文中的标准做法，但会将推理时间增加 3-10 倍。对于实时应用，通常只需要单次推理。`,
      code: [
        {
          lang: "python",
          code: `import torch

    ...
    content: [
        {
            ...
            body: `图像分类是计算机视觉最基础的任务：给定一张图片，判断它属于哪个预定义类别。从早期的手工特征（SIFT、HOG）到深度学习时代，核心思路始终是「提取有意义的视觉表征」。2009 年 ImageNet 的发布改变了游戏规则——它提供了超过 1400 万张标注图片和 1000 个类别，为模型训练提供了前所未有的数据规模。ImageNet Large Scale Visual Recognition Challenge（ILSVRC）成为了衡量算法能力的标准赛场，也直接推动了 CNN 架构的快速迭代。理解 ImageNet 的规模和多样性，是理解后续所有模型设计动机的起点。`,
            code: [
                {
                    lang: "python",
                    code: `import torch

...
**N-gram 模型**是最早的语言模型，基于马尔可夫假设：当前词的概率只依赖于前 n-1 个词。Bigram 假设 P(w₃ | w₁, w₂) ≈ P(w₃ | w₂)，Trigram 则依赖前两个词。概率通过计数频率估计：P(wᵢ | wᵢ₋₁) = count(wᵢ₋₁, wᵢ) / count(wᵢ₋₁)。N-gram 的问题是稀疏性——n 越大，需要的训练数据呈指数增长，且无法捕捉长距离依赖。
...
llm_judge_evaluation()`,
    ...
    style M fill:#c8e6c9`,
        ...
        },
        {
            ...
            body: `HuggingFace 的 Transformers 库提供了最完善的文本生成 API，支持所有主流解码策略。本节从最简单的 pipeline 开始，逐步深入到 generate() 方法的高级参数控制，最后展示如何实现自定义的解码逻辑。

**Pipeline 接口**是最简单的使用方式：一行代码完成模型加载、分词和生成。适合快速原型开发和简单任务。但 pipeline 隐藏了很多细节，对于需要精细控制解码策略的场景，应该直接使用 model.generate() 方法。

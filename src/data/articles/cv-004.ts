...
export const article: Article = {
    ...
    content: [
        {
            title: "1. 什么是卷积（1D → 2D）",
            body: `卷积（Convolution）是 CNN 最核心的运算，本质上是一种**带权滑动窗口**的局部操作。

**1D 卷积：** 假设输入信号 x = [1, 3, 5, 2, 4]，卷积核 k = [1, 0, -1]。卷积核在输入上从左到右滑动，每次覆盖 3 个元素，做对应位置的乘法并求和。第一个输出为 1×1 + 3×0 + 5×(-1) = -4，第二个输出为 3×1 + 5×0 + 2×(-1) = 1，依此类推。这个过程也叫**互相关（Cross-Correlation）**——深度学习框架实际计算的是互相关而非数学意义上的翻转卷积，但习惯上统称卷积。

**2D 卷积：** 将 1D 推广到图像（二维矩阵）。输入是一张 H×W 的图像（或多通道特征图），卷积核是 K×K 的二维权重矩阵。卷积核在图像上从左到右、从上到下滑动，每次计算覆盖区域内所有对应元素的乘积之和，生成输出特征图上的一个像素值。

**多通道卷积：** 实际图像有 RGB 三个通道，每个通道对应独立的 2D 卷积核，各通道的结果**逐元素相加**后得到单通道输出。如果有多个卷积核（输出多通道），则每个核独立计算一组结果。
...
print(apply_padding(img, 1, "replicate"))`,
  ...
  A -->|"P=2"| D["输出 7×7\n信息扩展"]`,
        ...
        {
            title: "5. 空洞卷积与感受野",
            body: `空洞卷积（Dilated / Atrous Convolution）在标准卷积核的元素之间插入「空洞」，在不增加参数量的前提下**扩大感受野**。空洞率（Dilation Rate）r 表示卷积核元素之间的间隔：r=1 为标准卷积，r=2 表示每隔 1 个像素取一个值。

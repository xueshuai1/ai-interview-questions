...
这些特性使得 **Agent 运行时治理**成为一个全新的工程领域。2024 年 arXiv 上发表的 Agent 自适应运行时治理研究明确指出，无法观察 Agent 内部状态时的监管是当前最核心的技术挑战之一。
...
两者互补，但运行时治理是当前工程实践中最薄弱的环节。大多数团队在部署 Agent 时，只做了静态的安全审查，却没有建立运行时的监控和约束机制。这意味着 Agent 一旦上线，其实际行为就完全不可控——这是一个极其危险的状态。`
        },
        {
            ...
            body: `Agent **可观测性**是运行时治理的基础。如果看不到 Agent 在做什么，就无法治理它。这里的看到不仅仅是记录 Agent 调用了哪些 API，而是理解它的意图、推理路径、决策依据和潜在风险。

### **可观测性**四层架构

一个完整的 Agent **可观测性**系统需要覆盖四个层面，从最基础的操作记录到最深层的意图推断。

第一层是**调用层**（**Invocation Layer**）。记录 Agent 的每一次外部操作，包括工具调用、API 请求、文件读写。这是最基本也最容易实现的层面。关键指标包括工具调用频率和类型分布、API 调用的目标地址和参数模式、文件操作的路径范围和权限级别、网络请求的目标域名和数据量。

第二层是**推理层**（**Reasoning Layer**）。通过 ReAct、Chain-of-Thought 等模式，强制 Agent 输出其推理过程。虽然这不等同于真正观察到 LLM 内部状态，但可以提供足够的透明度来识别异常行为。实现方式包括要求 Agent 在每个步骤输出思考行动观察日志、对推理日志进行实时语义分析以识别可疑模式、对比 Agent 声明的意图与其实际执行的操作。

第三层是**状态层**（**State Layer**）。跟踪 Agent 在整个任务生命周期中的状态变化，包括当前任务进度和子目标完成情况、内存中存储的上下文信息即短期记忆、向量数据库中的长期记忆访问模式、Agent 的自我评估和信心分数。

第四层是**意图层**（**Intent Layer**）。这是最具挑战性的层面，尝试推断 Agent 的真实意图而非其声称的意图。当前可行的方法包括**行为一致性检测**即对比 Agent 的多步行为判断是否存在目标漂移、**谄媚倾向**分析即检测 Agent 是否为了迎合用户而牺牲准确性、**异常模式识别**即使用机器学习模型识别偏离正常行为模式的 Agent 操作。

### **可观测性**的实施优先级

对于刚开始建设 Agent 治理的团队，建议按以下优先级推进：

第一阶段（必须）：实现调用层**可观测性**，确保所有工具调用都有完整日志。

第二阶段（重要）：实现推理层**可观测性**，通过结构化提示强制 Agent 输出推理过程。

第三阶段（推荐）：实现状态层**可观测性**，建立 Agent 行为基线。

第四阶段（进阶）：实现意图层**可观测性**，部署异常检测和谄媚分析模型。`
        },
        {
            title: "3. 行为约束机制——让 Agent 不能做什么",
            body: `**可观测性**告诉你 Agent 在做什么，**行为约束**确保 Agent 不能做不该做的事。**行为约束**是运行时治理的核心防御手段，需要在 Agent 执行任何操作之前进行实时检查。

### 约束引擎的三层设计

一个健壮的**行为约束**系统应该采用三层防御架构，每层针对不同类型的风险。

第一层是**预检约束**（Pre-flight Constraints）。在 Agent 执行操作之前，对其计划进行检查。核心机制包括**白名单机制**即只允许 Agent 调用预定义的工具集合、参数验证即检查 API 调用的参数是否在安全范围内、**速率限制**即防止 Agent 在短时间内发起大量请求、**资源配额**即限制 Agent 可以消耗的 Token 数量和 API 调用次数。

第二层是**运行时约束**（Runtime Constraints）。在 Agent 执行操作的过程中，进行实时监控和干预。核心机制包括输出过滤器即检查 Agent 的响应内容是否包含敏感信息、操作拦截器即当 Agent 尝试执行高危操作时需要人工确认、上下文注入即向 Agent 的系统提示中注入安全约束使其自我约束。

第三层是**后验约束**（Post-hoc Constraints）。在 Agent 完成操作之后，对结果进行审查。核心机制包括结果验证即检查 Agent 的输出是否符合预期格式和内容要求、影响评估即评估 Agent 操作对系统状态的实际影响、审计日志即记录所有操作及其结果用于事后分析和改进。`,
            code: [
                {
                    lang: "python",
...
print(f"允许: {result.allowed}, 原因: {result.reason}")`
        ...
        },
        {
            ...
            body: `**行为约束**不是一成不变的。随着对 Agent 行为模式理解的深入，约束策略应该持续演进。
...
**过度约束**是最大的陷阱。约束过于严格会导致 Agent 无法完成有用任务，用户会觉得 Agent 太笨、太慢、太受限。平衡安全性和实用性是关键。一个经验法则是：约束应该阻止 Agent 做危险的事，但不应该阻止 Agent 做有用的事。

**告警疲劳**是另一个常见问题。过多的告警会使运维人员忽视真正的安全问题。应该按严重度分级，只告警真正需要关注的事件。建议采用分级告警策略：低风险事件只记录日志，中风险事件发送通知，高风险事件触发即时告警。
...
检测**谄媚倾向**需要建立量化度量体系。方法一是**事实一致性评分**，构建一个事实核查管道，对比 Agent 的输出与已知事实。方法二是**对抗性提示测试**，在 Agent 不知情的情况下注入轻微的偏见提示，观察 Agent 是否会放大该偏见。方法三是**多 Agent 交叉验证**，让多个独立 Agent 对同一问题给出回答，对比它们的一致性。`,
            code: [
                {
                    lang: "python",
...
print(f"纠正回复: {r2['sycophancy_score']} ({r2['verdict']})")`
        ...
        },
        {
            ...
            body: `即使有完善的**可观测性**和约束系统，Agent 仍可能突破防线。**紧急制动**（Emergency Brake）是运行时治理的最后一道防线，当检测到 Agent 行为严重偏离预期时，立即中止其执行并恢复到安全状态。
...
制动必须无法被制动对象控制。确保**紧急制动**系统本身不会被 Agent 篡改或绕过，制动系统的代码应该运行在 Agent 无法访问的隔离环境中。
...
阶段一基础可见性（第 1-2 周）。实现调用层**可观测性**，记录所有工具调用和 API 请求。建立基本日志系统，确保所有 Agent 行为都有迹可循。设置关键指标的告警阈值。

阶段二主动约束（第 3-4 周）。部署约束引擎，实施白名单、**速率限制**和**资源配额**。建立高危操作审批流程。开始收集行为基线数据。

阶段三智能监控（第 5-8 周）。实现推理层**可观测性**，通过 ReAct 模式获取推理日志。部署谄媚检测模块。建立异常行为检测模型。

阶段四自主治理（第 9-12 周）。实现**紧急制动**系统。建立自动化的约束策略优化循环。将治理集成到 CI/CD 流程中。

### 治理仪表板的关键指标

一个完整的 Agent 治理仪表板应该包含安全性指标、可靠性指标、公平性指标和效率指标四大类。安全性指标包括约束违反次数（告警阈值大于每小时 5 次）和**紧急制动**触发次数（告警阈值为 0 次）。可靠性指标包括任务完成率（告警阈值低于 80%）和异常行为检测率（告警阈值高于 10%）。公平性指标包括**谄媚倾向**评分（告警阈值高于 0.6）。效率指标包括 Token 消耗与任务预算比（告警阈值高于 120%）和工具调用有效率（告警阈值低于 50%）。`
        },
        {
            title: "8. Agent 运行时治理的工具生态",
            body: `**Agent 运行时治理**是一个新兴领域，但已经有一些开源和商业工具开始提供相关能力。

### 开源工具

**Langfuse** 提供 LLM 追踪、延迟监控和成本分析功能，适用于**调用层**和推理层**可观测性**，优势是开源、可自部署、社区活跃。

**LangSmith** 提供 LLM 应用开发的全流程追踪，适用于推理链分析、测试和调试，优势是与 LangChain 生态深度集成。

**Arize Phoenix** 提供 LLM 评估、追踪和**可观测性**，适用于质量维度和数据维度监控，优势是开源且支持本地部署。

**OpenLIT** 提供 OpenTelemetry 标准的 LLM **可观测性**，适用于与现有 APM 系统集成，优势是标准协议和厂商中立。

### 治理专用框架

**AgentGuard** 是一个研究原型的 Agent **行为约束**和运行时监控框架，适用于多层约束引擎和**紧急制动**，目前处于学术研究阶段尚未生产就绪。

Constitutional AI Runtime 是基于宪法的**运行时约束**框架，适用于将 AI 安全原则编码为运行时规则，是 Anthropic 研究框架的延伸实现。
...
Sycophancy in LLMs: Quantifying and Mitigating Model Yes-Manning（arXiv, 2024）首次系统量化 LLM **谄媚倾向**的研究。

Agent Adaptive Runtime Governance: Monitoring Autonomous Systems Without Internal Observability（arXiv cs.AI, 2024）**Agent 运行时治理**框架的开创性研究。
...
本系列其他文章包括 agent-001 AI Agent 入门从概念到实现、agent-002 Multi-Agent 系统设计与协作、agent-030 AI Agent 权限管理与访问控制最佳实践、ethics-007 AI 治理框架从原则到制度、aiobs-001 AI **可观测性**与可靠性工程。`
        },
        {
            title: "架构图示 1",
            mermaid: `graph TD

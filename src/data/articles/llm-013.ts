...
class MoELayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        },
        {
            ...
            body: `MoE 训练中最核心的挑战之一是**负载均衡（Load Balancing）**。理想情况下，每个专家应该处理大致相等数量的 token，这样才能充分利用所有计算资源。但在训练初期，路由器的权重是随机初始化的，很可能导致某些专家被过度使用，而另一些专家几乎不被激活——这就是所谓的"专家坍塌"（Expert Collapse）。

专家坍塌是一个自我强化的恶性循环：如果某个专家在训练初期随机获得了较高的门控分数，它会接收到更多 token 的梯度更新，从而变得"更强"；而较弱的专家因为接收不到足够的训练信号，永远无法提升，最终完全被路由器忽略。

为了解决这个问题，MoE 训练中引入了**辅助损失（Auxiliary Loss）**，也称为负载均衡损失。最常用的是 Switch Transformer 提出的负载均衡损失：对于每个 MoE 层，计算每个专家被选中的频率 fᵢ 和平均门控权重 Pᵢ，然后最小化 N·Σ(fᵢ·Pᵢ)，其中 N 是专家数量。这个损失的直觉是：如果所有专家被均匀使用（fᵢ = 1/N）且平均权重也均匀（Pᵢ = 1/N），则损失达到最小值 1。
...
**通信优化技术：** DeepSpeed 和 Megatron-LM 等框架提供了多种优化技术。例如，使用**拓扑感知的 All-to-All 通信**来减少跨节点通信开销；使用**专家缓存**来减少频繁的 GPU 间数据传输；使用**分层专家分配**来确保高频通信的专家在同一个节点上。

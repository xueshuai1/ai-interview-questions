...
Self-Evolving AI Agent（自进化智能体）的核心突破在于**能力边界的动态扩展**。它具备三个核心特征：
...
这与生物界的进化过程高度相似：通过变异（探索新策略）、选择（保留有效策略）、遗传（固化到长期记忆）实现能力增长。`,
    ...
    C1 -.-> C1a[seed扩展技能树]
    D1 -.-> D1a[GEP基因组进化]
    style A fill:#3730a3,color:#f1f5f9
...
Hermes Agent 的核心创新在于**闭环学习系统**（Closed Learning Loop）：
...
Hermes Agent 支持在 $5 VPS 上运行，通过 Telegram/Discord/Slack 等渠道交互，实现了**低成本部署 + 多端连续性**。`,
      code: [
        {
          lang: 'python',
...
技能树路线将 Agent 的能力组织为**可生长的树状结构**：根节点是基础能力，每完成新类型任务就生长出新的技能分支。核心循环：**新任务 → 能力评估 → 技能创建/扩展现有技能 → 验证 → 固化到技能树**。
...
遗传进化路线将 Agent 的决策逻辑编码为**基因组（Genome）**，通过遗传编程（GP）或基因表达编程（GEP）进行进化。核心循环：**种群初始化 → 适应度评估 → 选择 → 交叉/变异 → 新一代种群**。

代表项目：**Evolver**（6,071 stars，本周新增 4,032 stars）。

### 5.2 GEP 的关键优势

GEP 的核心优势是**基因组固定长度保证遗传操作安全，表达树可变长度保证行为多样性**。这使得进化过程既稳定又富有探索性。`,
      mermaid: `graph LR
    ...
    style D fill:#3730a3,color:#f1f5f9`
    },
    {
      title: '六、GEP 代码实现',

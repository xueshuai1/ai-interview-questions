    ...
    content: [
        {
            ...
            body: `全连接网络（Fully Connected Network）处理图像时面临两个致命问题。第一是**参数爆炸**：一张 224×224 的 RGB 图片展平后是 150,528 维输入，即使第一层只有 100 个神经元，参数量也高达 1500 万，远超合理范围。第二是**空间信息丢失**：展平操作（Flatten）将二维像素矩阵变成一维向量，破坏了像素间的局部邻接关系——相邻像素在展平后可能相距甚远，网络无法利用「附近的像素往往语义相关」这一视觉先验。

CNN 通过两个核心设计解决这些问题：**局部感受野**（Local Receptive Field）和**权重共享**（Weight Sharing）。局部感受野意味着每个神经元只看输入的一小块区域（如 3×3），而不是全局；权重共享意味着同一个卷积核在整张图上滑动复用，无论检测到的特征出现在图片的哪个位置，都用同一组参数识别。这两个机制将参数量从 O(n²) 压缩到 O(k²)，同时天然保留了空间结构信息。
...
# 像素 3(index=2) 和像素 4(index=3) 虽然相邻，但语义关联被削弱` },
    ...
    E -. "参数高效" .-> G["泛化能力强"]`,
        ...
        },
        {
            ...
            body: `卷积（Convolution）是 CNN 的核心运算。在深度学习中，我们实际执行的是**互相关**（Cross-Correlation）——虽然严格意义上的卷积需要将核翻转 180°，但网络会通过学习自动调整核的权重，因此翻转与否不影响表达能力。

卷积运算包含三个关键超参数：**卷积核大小**（Kernel Size）、**步长**（Stride）和**填充**（Padding）。卷积核大小决定了感受野的范围，3×3 是最常见的选择（两个 3×3 卷积的等效感受野等于一个 5×5 卷积，但参数量更少、非线性更强）。步长控制核每次滑动的距离，步长为 2 时输出尺寸减半，常用于下采样。填充用于在输入四周补零，保持输出尺寸不变（Same Padding）或允许核访问边缘区域。
...
# 输出: 30x30, 32x32, 16x16` },
    ...
    G --> H["每个位置 = 核·局部区域求和"]`,
        ...
        {
            title: "3. 池化层：降维与不变性",
            body: `池化（Pooling）层是 CNN 中的下采样操作，它沿着空间维度（高度和宽度）滑动窗口，对每个窗口内的值做聚合运算。池化的主要目的有两个：**降低特征图的空间尺寸**从而减少后续层的计算量和参数，以及**增强平移不变性**（Translation Invariance）——即目标在图片中轻微移动时，池化后的输出变化不大。

最常见的池化方式是**最大池化**（Max Pooling），它取窗口内的最大值。这相当于在局部区域内做「特征存在性检测」：只要某个强特征（如边缘）出现在窗口内的任何位置，最大池化就能捕获到它。另一种方式是**平均池化**（Average Pooling），它计算窗口内的平均值，更适合保留整体背景信息。现代 CNN 中最大池化更常见，因为它能更好地保留显著特征。
...
学习率设置是微调成功的关键。通常对新添加的分类头用较大的学习率（如 1e-3），对预训练的主干网络用较小的学习率（如 1e-5 或 1e-4），这种**分层学习率**（Discriminative Learning Rate）策略能防止微调过程破坏已经学到的良好特征。`,
            code: [
                { lang: "python", code: `import torch
import torch.nn as nn
import torchvision.models as models

...
训练流程的核心路径可以分为五个阶段：**数据准备**是基石，决定了模型知识的广度和质量上限；**预训练**是核心，让模型从海量文本中学习语言规律和世界知识，产出基座模型（Base Model）；**有监督微调（SFT）**让基座模型学会遵循指令，从"会说话"变成"能帮忙"；**奖励模型训练**用人类偏好数据训练一个评判者；**强化学习对齐（RLHF/DPO）**则用奖励模型或直接偏好优化来微调模型，使其输出与人类价值观一致。
...
现代 LLM 的训练数据是高度混合的：**网页文本**（如 Common Crawl）提供了最广覆盖的语言知识，但噪声最大；**代码数据**（如 GitHub）对模型的逻辑推理能力有显著提升；**书籍**提供了结构化的长文本，有助于模型学习深层推理和叙事结构；**学术论文**则注入专业领域的精确知识。每种数据源都有其独特的价值，但也需要针对性的清洗策略。

数据清洗是整个管线中最关键也最容易被低估的环节。**去重**（Deduplication）防止模型记忆训练数据，常用的 MinHash 算法可以在 O(n) 复杂度内检测近似重复；**质量过滤**通过语言分类器、困惑度（Perplexity）阈值、以及启发式规则筛除低质量文本；**PII 去除**则是合规性要求，需要识别并抹除个人身份信息。这些步骤看似繁琐，但直接决定了模型是否会"背诵"训练数据、输出有害内容或泄露隐私。`,
        code: [
          {
            lang: "python",
...
SFT 的本质是监督学习：给定一组（指令，期望回答）的数据对，通过标准的多类别交叉熵损失函数微调模型。但这里的关键不在于算法本身，而在于**数据质量**。指令数据的质量直接决定了模型的行为表现——10,000 条精心构造的高质量指令数据，效果远胜于 100,000 条从网络爬取的噪声数据。
...
传统的 RLHF 流程包含三个步骤：首先，让 SFT 模型对同一指令生成多个回答，由人类标注者对这些回答进行排序（偏好标注）；然后，用这些排序数据训练一个**奖励模型（Reward Model）**，让它学会给任意回答打分；最后，使用 **PPO（Proximal Policy Optimization）** 算法，以奖励模型的评分为优化目标，微调策略模型。PPO 的优势在于它限制了每次更新的幅度，避免策略模型为了获得高分而"走捷径"——比如生成极端讨好的回答而牺牲事实准确性。
...
评估体系通常分为多个维度：**知识理解**用 MMLU（Massive Multitask Language Understanding）测试模型在 57 个学科上的表现；**数学推理**用 GSM8K 测试模型解决小学到初中数学题的能力；**代码能力**用 HumanEval 测试模型生成正确 Python 函数的能力；**指令遵循**用 IFEval 测试模型是否能严格遵守格式约束；**安全性**用 TruthfulQA 和 ToxiGen 测试模型是否会产生虚假或有害内容。综合这些指标，才能对模型的能力有一个全面的认识。

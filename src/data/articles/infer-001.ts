...
推理加速不再只是「锦上添花」，而是 AI 能否大规模商用的**生死线**。`,
      mermaid: `graph TD
    A["LLM 推理加速"] --> B["算法层优化"]
    A --> C["架构层优化"]
    A --> D["系统层优化"]
...
传统推测解码中，草稿模型也是自回归生成的——每次生成一个 token，然后下一个。DFlash 的突破在于**块扩散（Block Diffusion）**：草稿模型一次性生成整个 token 块，而非逐 token 生成。
...
DFlash 每轮只验证**一条**草稿路径。DDTree 的关键突破是从块分布中构建**草稿树（draft tree）**：

1. 从块扩散草稿器的每位置分布直接构建多路径草稿树
2. 使用最优优先堆算法选择最可能匹配目标模型的延续
3. 目标模型在**单次前向传播**中验证整棵树
4. 使用仅祖先注意力掩码实现高效验证
...
前两种方案都是「先推测后验证」——错误步骤会传播，导致后续 token 全部浪费。SpecGuard 引入了**步骤级验证**：

1. 每步采样多个候选
2. 使用**注意力 Grounding 分数**和 **Log-probability 分数**的轻量级集成进行步骤级验证
...
推理部署消耗大量算力的根本问题是：标准 Transformer 的参数越多，推理时的内存占用越大。Parcae 的思路是**让模型更紧凑，但在推理时执行更多计算**。
...
- **扩展新维度**：循环为 AI 研究提供了不依赖更多硬件的第三条扩展路径（除了参数和数据）`,
    ...
    {
      lang: "text",
      code: `输入 → [Prelude] → 潜在状态 e → [循环块 × T 次] → 最终状态 → [Coda] → 输出
`,
    },
    {
      lang: "text",
      code: `h_{t+1} = Ā · h_t + B̄ · e + R̄(h_t, e)
`,
    }
      ]
    },

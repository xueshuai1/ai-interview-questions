...
**对比 FP16：** FP8 将显存占用和计算量**减半**，精度损失在 LLM 推理中通常 <1%（perplexity 差异 <0.1）。对于 70B 模型，FP16 需要 140GB 显存，FP8 只需 70GB——从需要 8×H100 降到 4×H100，成本直接减半。
...
DeepGEMM 的关键创新在于**细粒度缩放（Fine-grained Scaling）**。传统 FP8 量化使用 per-tensor 或 per-channel 缩放——整个张量或每个通道共享一个缩放因子。但 LLM 的激活值在不同 token、不同位置上的分布差异巨大，粗粒度缩放会导致精度损失。
...
**关键结论：** H100 及以上（或 B200/GB200）是使用 FP8 推理的最低硬件要求。如果你的部署环境是 A100 或更老的 GPU，FP8 只能退化为软件模拟，反而会**变慢**。
...
3. 如果你是推理服务提供商，FP8 是你的**成本竞争力**核心
4. 如果你是模型开发者，在模型发布时**同时提供 FP8 量化版本**

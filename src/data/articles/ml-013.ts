...
plot_decision_boundary(bagging, X_test, y_test, "Bagging (50 棵树)")`,
    ...
    I --> J["最终预测"]`,
        ...
        },
        {
            ...
            body: `Boosting 与 Bagging 的哲学完全不同：Bagging 是「民主投票」，每个模型独立训练、平等投票；Boosting 是「接力赛」，模型一个接一个地训练，每个新模型都专注于前一个模型犯错的样本。
...
print(f"最高测试准确率: {test_scores[best_idx]:.4f}")`,
    ...
    style F fill:#14532d`,
        ...
        },
        {
            ...
            body: `Stacking（Stacked Generalization）是 Wolpert 在 1992 年提出的集成策略，它比 Bagging 和 Boosting 更进一步：不是简单地投票或加权平均，而是训练一个「元学习器」（Meta-Learner）来学习如何最佳地组合基模型的预测。
...
for name, pred in strategies.items():
    ...
    style G fill:#14532d`,
        ...
        },
        {
            ...
            body: `集成学习的核心前提是「多样性」（Diversity）。如果所有基模型都一模一样，那么集成后的结果和单个模型没有任何区别——10 个完全相同的专家一起投票，和 1 个专家单独决策的效果完全等价。

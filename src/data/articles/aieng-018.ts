...
Claude Code 在处理长对话时，采用了**动态上下文截断策略**来优化 token 使用。但在 2-3 月的一次更新中，截断阈值被错误调整，导致：
...
Claude Code 通过一系列工具调用（文件读取、代码搜索、终端执行等）来完成编程任务。在一次底层重构中，**工具调用的重试逻辑**引入了一个边界条件 bug：
...
Claude Code 的系统提示词（system prompt）在多次迭代中发生了**非预期漂移**：
...
这种叠加效应在 Anthropic 内部评测中未被发现，因为每个缺陷单独看都在「可接受」范围内，但叠加后用户体验急剧恶化。`,
    ...
    },
    {
      ...
      body: `Anthropic 事后分析中最有价值的部分之一是**坦诚地解释了为什么内部评测未能提前发现这些问题**。这揭示了 AI 编程工具评测领域的一个普遍挑战。
...
`,
    },
    {
      ...
      body: `除了会话级评测，还需要在 CI/CD 流水线中建立**AI 质量门禁**，确保每次代码变更都不会导致质量退化。

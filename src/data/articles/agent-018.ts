...
LLM 安全主要关注输出内容（是否生成有害文本），而 Agent 安全需要考虑**执行层面**的风险：
...
Agent 不仅仅是「会说话的模型」，它是**拥有工具、权限和自主决策能力的执行体**。一旦安全防线被突破，后果不再是「输出了不该说的话」，而是「执行了不该做的事」。`,
      mermaid: `graph TD
...
|------------|---------------|---------|---------|
| **S**poofing（欺骗） | 冒充用户或系统身份 | 伪造用户指令、伪造工具返回 | 身份验证、数字签名 |
| **T**ampering（篡改） | 修改 Prompt、工具配置或执行结果 | Prompt 注入、工具输出劫持 | 输入验证、完整性校验 |
| **R**epudiation（抵赖） | Agent 执行操作后无法追溯 | 缺乏审计日志 | 完整的操作审计链 |
| **I**nformation Disclosure（信息泄露） | Agent 访问并泄露敏感数据 | 通过工具调用读取机密文件 | 最小权限、数据脱敏 |
| **D**enial of Service（拒绝服务） | 消耗 Agent 计算资源或工具配额 | 无限循环调用、超大输入 | 资源限制、超时机制 |
| **E**levation of Privilege（权限提升） | Agent 获取超出预期的权限 | 工具链跳转、子 Agent 权限继承 | 权限隔离、沙箱 |
...
\`\`\``,
    },
    {
      ...
      body: `自动化测试覆盖已知的攻击模式，但红队测试模拟的是**真实攻击者的思维方式**——他们会尝试你从未想过的攻击路径。
...
AI Agent 安全评估不是一个「功能」，而是一个**持续的系统工程**。它需要：

1. **威胁建模**作为起点——理解你的 Agent 面临什么
2. **自动化测试**作为基线——确保已知风险被覆盖
3. **红队测试**作为补充——发现未知的攻击路径
4. **持续监控**作为保障——在运行时检测和响应威胁
5. **安全治理**作为基础——让安全成为团队文化

...
多模态学习的核心挑战在于**模态对齐（Modality Alignment）**——将不同模态的信息映射到同一个语义空间。例如，「一只棕色的狗在草地上奔跑」这句话和一张对应的图片，在人类大脑中唤起的是相同的概念，但对模型而言，文本是离散 token 序列，图像是连续像素矩阵，两者的数学表示截然不同。模态对齐的目标就是让这两种表示在某个高维空间中彼此靠近。

多模态模型的应用场景极其广泛：**图文检索**（输入文字找图片，或输入图片找描述）、**视觉问答**（VQA，看图回答问题）、**图像描述生成**（Image Captioning）、**多模态对话**（既能聊天又能看图）、**具身智能**（机器人结合视觉和语言指令执行任务）。GPT-4V、Gemini、Claude 的视觉能力，本质上都是多模态学习的产物。理解多模态学习的基础概念，是掌握后续 CLIP、LLaVA 等具体技术的前提。`,
        code: [
          {
            lang: "python",
...
具体而言，CLIP 使用一个**图像编码器**（ViT 或 ResNet）和一个**文本编码器**（Transformer）分别提取图像和文本的特征向量。然后，通过计算所有图像-文本对的余弦相似度，构建一个 N×N 的相似度矩阵。对角线上的元素是正样本对（匹配的图文对）的相似度，非对角线元素是负样本对（不匹配的图文对）的相似度。训练目标就是最大化对角线元素的相似度，同时最小化非对角线元素的相似度。

这种设计的关键优势在于**可扩展性**：CLIP 不需要人工标注的类别标签，而是利用互联网上自然存在的图文对（网页上的图片和它的 alt-text 或 surrounding text）。OpenAI 最初训练 CLIP 时使用了 4 亿对图文数据，这使得 CLIP 获得了令人惊艳的零样本（Zero-Shot）分类能力——即使从未见过某个类别的标注数据，也能通过文本 prompt 进行分类。`,
        code: [
          {
            lang: "python",
            code: `# CLIP 对比损失计算
...
这种设计的美妙之处在于**最小化修改**——LLaVA 不需要对 LLaMA 做任何架构层面的改动，只需要在输入层注入视觉信息。这使得 LLaVA 可以复用 LLaMA 的全部训练基础设施（训练脚本、推理引擎、量化工具等），大大降低了开发和部署成本。LLaVA 证明了：一个足够强大的 LLM 加上合理的视觉注入方式，就能实现出色的多模态理解能力。`,
        code: [
          {
            lang: "python",
...
指令微调过程中有几个关键技术细节值得注意：**对话模板**需要正确区分图像 token、系统提示、用户指令和助手回答；**损失计算**只对助手回答部分计算（不计算图像 token 和用户输入的损失）；**学习率**要比预训练低很多（通常 2e-5 量级），以避免灾难性遗忘；**批大小**受限于视觉 token 带来的额外显存开销，通常需要更激进的梯度累积策略。`,
        code: [
          {
            lang: "python",
            code: `# 多模态指令数据格式

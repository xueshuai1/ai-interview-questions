...
- **33-67% 的文档**存在至少一次传递性违反（即 A>B>C 但 C>A 的逻辑矛盾）
- **相关性的评估最为可靠**，而**流畅性和一致性的评估几乎不可靠**
- **评估标准的选择比使用哪个评委模型更重要**
...
然而，现有评估几乎都聚焦于**聚合层面的相关性**（aggregate correlation）——即 LLM 评委的平均评分是否与人类平均评分一致。这忽略了一个关键问题：
...
如果评委判断 A > B、B > C 但 C > A，就构成了**传递性违反**——一个**定向三元环**（directed 3-cycle），这在逻辑上是不可能的。
...
因为聚合指标会被「大多数简单样本的正确判断」所稀释。一篇文档可能有 28 对比较（8 选 2），其中 27 对满足传递性，只有 1 对违反——聚合违反率很低，但那 1 对违反意味着评委对这篇文档的判断存在**根本性的逻辑矛盾**。
...
更糟的是，传递性违反在不同评估标准间的分布极不均匀——某些标准上的违反率远高于其他标准，这说明**评估任务本身的性质**对可靠性有决定性影响。`,
    },
    {
      title: "三、诊断工具箱 #2：保形预测集",
      body: `### 3.1 什么是保形预测？

**保形预测（Conformal Prediction）** 是一种统计学框架，能够为任何机器学习模型的预测提供**有理论保证的不确定性量化**。

核心思想：不输出一个点预测（如「这个摘要是 4 分」），而是输出一个**预测集**（如「这个摘要的分数在 3-5 分之间」），并保证真实值以 ≥ (1-α) 的概率落在该集合内。
...
研究者在 1,918 个样本上验证了这一假设：预测集宽度与逐例可靠性之间存在**极强的正相关**（Spearman ρ = +0.576，p < 10⁻¹⁰⁰）。

### 3.3 跨评委的一致性验证

更令人信服的是，研究者在 4 个不同的 LLM 评委之间发现：**预测集宽度在不同评委间具有稳定的相关性（r̄ = 0.32-0.38）**。

这意味着什么？意味着预测集宽度捕捉的是**文档本身的难度**，而不是某个评委特有的噪声。

如果一份摘要对所有评委来说都很难判断，那么所有评委都会输出宽集合。这证明了保形预测集宽度是一个**客观的可靠性指标**，而非模型特定的 artifacts。
...
\`\`\`python
import numpy as np
from conformal_inference import SplitConformalPredictor

# 1. 校准阶段
calibration_scores = []
for sample in calibration_set:
    llm_score = llm_judge(sample)  # LLM 的评分
    human_score = sample.human_label  # 人类标注的真实分数
    # 非一致性分数 = |LLM评分 - 人类评分|
    calibration_scores.append(abs(llm_score - human_score))
...
# 3. 预测阶段
def predict_with_uncertainty(new_sample, q_hat):
    llm_score = llm_judge(new_sample)
    # 预测集 = [llm_score - q_hat, llm_score + q_hat] ∩ [1, 5]
    lower = max(1, int(np.ceil(llm_score - q_hat)))
    upper = min(5, int(np.floor(llm_score + q_hat)))
...
5. **人类-LLM 混合评估**：在低可靠性样本上自动回退到人类评估，实现成本与质量的最佳平衡`,
    ...
    {
      title: "七、总结与行动建议",
      body: `这篇论文为 LLM-as-Judge 的可靠性问题提供了一套**可操作的诊断工具**。它告诉我们两个重要事实：

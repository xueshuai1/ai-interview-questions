...
2026 年，记忆不再是「锦上添花」，而是 Agent 的**核心竞争力**。没有记忆的 Agent 就像没有海马体的人类——能思考，但无法成长。

记忆系统的架构正在经历从「简单追加」到「智能管理」的范式转变。早期的 Agent 记忆就是简单的对话历史追加，很快就遇到上下文窗口限制。现在的记忆系统采用**分层架构**——短期工作记忆、中期情景记忆、长期语义记忆，每一层有不同的存储策略和检索机制。`,
      mermaid: `graph TD
    A["用户输入"] --> B["感知层\nPerception"]
    B --> C["工作记忆\nWorking Memory\n(当前会话)"]
    C --> D["情景记忆\nEpisodic Memory\n(近期事件)"]
    ...
    },
    {
      ...
      body: `受人类认知科学启发，现代 AI Agent 记忆系统普遍采用**四层分层架构**。每一层有不同的存储介质、检索策略和生命周期。

### 2.1 工作记忆（Working Memory）—— Agent 的「注意力焦点」

工作记忆对应人类的「短时记忆」，存储当前会话的上下文信息。它的特点是**高速读写、容量有限、快速衰减**。
...
情景记忆存储 Agent 的**具体经历和事件**。它回答的问题是：「这个 Agent 之前做过什么？」
...
**技术实现**：情景记忆通常使用**向量数据库**存储。每条经历被编码为向量嵌入（embedding），通过语义相似度检索相关历史。

### 2.3 语义记忆（Semantic Memory）—— Agent 的「知识库」

语义记忆存储**抽象的、去情境化的知识**。它回答的问题是：「这个 Agent 知道什么？」
...
程序记忆存储**如何做事情的技能和方法**。它回答的问题是：「这个 Agent 擅长做什么？」
...
Claude-Mem 和 GenericAgent 的核心创新之一就是**程序记忆的自动化构建**——Agent 在执行任务过程中自动发现并固化新的技能。`,
      mermaid: `graph LR
    subgraph "记忆四层架构"
        A["工作记忆\n• 当前会话上下文\n• 滑动窗口管理\n• 毫秒级访问"] 
        B["情景记忆\n• 历史事件记录\n• 向量数据库存储\n• 语义相似度检索"]
...
记忆压缩的本质是**信息论中的率失真问题**——在给定存储预算下，最大化保留信息的效用。`,
    },
    {
...
Claude-Mem 的核心创新是**自动捕获 + AI 压缩 + 上下文注入**的闭环：

1. **捕获阶段**：自动记录 Claude Code 在编码会话中的所有操作——文件修改、终端命令、错误信息、用户反馈
2. **压缩阶段**：使用 Claude 自身的 agent-sdk，将原始操作流压缩为结构化的记忆条目。不是简单摘要，而是提取**关键决策、问题模式、解决方案模板**
3. **注入阶段**：在新会话开始时，根据当前任务语义相似度，从记忆库中检索最相关的条目注入到系统提示中

这种机制的关键在于**压缩不是有损的**——它保留了决策的上下文和推理过程，而不仅仅是结果。
...
- 如果你的 Agent 需要**记住用户偏好和历史对话** → 至少实现工作记忆 + 情景记忆
- 如果你的 Agent 需要**积累领域知识** → 加入语义记忆层，使用知识图谱
- 如果你的 Agent 需要**自主学习和成长** → 实现程序记忆，让 Agent 自动发现和固化技能

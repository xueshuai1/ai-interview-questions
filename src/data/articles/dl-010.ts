    ...
    content: [
        {
            ...
            body: `神经网络训练的第一步不是前向传播，而是**权重初始化**。初始化方案直接决定了训练能否顺利启动——糟糕的初始化会导致梯度消失（Gradient Vanishing）或梯度爆炸（Gradient Exploding），使模型在训练初期就「死掉」。
...
# 第 10 层: std=0.672451 ← 保持稳定` },
    ...
    style I fill:#14532d`,
        ...
        },
        {
            ...
            body: `Xavier 初始化由 Xavier Glorot 和 Yoshua Bengio 在 2010 年的论文《Understanding the difficulty of training deep feedforward neural networks》中提出。它是最早系统解决深度网络初始化问题的方案之一，核心思想是**让每一层的激活值方差和梯度方差都保持为 1**。
...
class HeInitializedNet(nn.Module):
    ...
    style G fill:#14532d`,
        ...
        },
        {
            ...
            body: `正交初始化（Orthogonal Initialization）将权重矩阵初始化为**正交矩阵**（满足 W^T W = I）。它的核心优势是：正交变换不改变向量的范数，即 ||Wx|| = ||x||。这意味着无论网络多深，前向传播中的信号幅度既不会爆炸也不会消失——从理论上完美解决了梯度消失/爆炸问题。
...
class LayerScaledTransformer(nn.Module):
    ...
    style H fill:#14532d`,
        ...
        },
        {
            ...
            body: `理解初始化如何影响训练动态，需要从**激活值分布**和**梯度分布**两个维度来分析。好的初始化应该让激活值保持良好的统计特性（均值接近 0、方差稳定），同时让梯度在反向传播中既不过大也不过小。
...
实验设计遵循以下原则：**固定所有超参数**（学习率 0.01、SGD 动量 0.9、weight decay 5e-4、batch size 128），仅改变初始化方案；**每种方案重复 3 次**取平均值以减少随机性影响；**记录训练 loss 曲线和测试精度**，同时监控每层激活值的标准差变化。
